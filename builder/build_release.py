...
def clean_outputs(
    ...
    # Installer/package/update outputs are always regenerated.
    targets = [INSTALLER_OUTPUT_DIR, PACKAGE_OUTPUT_DIR, UPDATES_OUTPUT_DIR]
...
def build_pyinstaller(force_clean: bool = False) -> Path:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  staging tree (stage/core/) are preserved between runs.\n"
...
def main() -> int:
    args = parse_args()
    if args.clean_only:
        clean_outputs(preserve_build=False)
        return 0

    # On incremental runs preserve both the PyInstaller work-cache (BUILD_DIR)
    # and the staged bundle (STAGE_DIR/core) so that:
    ...
    clean_outputs(
        preserve_dist=args.skip_pyinstaller,
    ...
    if not args.skip_pyinstaller:
        ...
        source_dir = build_pyinstaller(force_clean=args.clean_build)
