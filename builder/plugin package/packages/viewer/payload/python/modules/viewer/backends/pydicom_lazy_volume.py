...
class PyDicomLazyVolume(QObject):
    ...
    def grow(self) -> int:
        ...
        After ``refresh_file_list()`` the backend re-sorts ``_slices`` by
        instance number (acquisition order).  Newly downloaded files may sort
        ...
        try:
            ...
            if any(o != n for o, n in mapping.items()):
                old_to_new = mapping
                logger.info(
                    "pydicom-lazy grow: instance-number re-order detected, remapping %d decoded slices",
