...
class DownloadManagerWidget(QWidget):
    ...
    def _pause_all_active_downloads(self) -> None:
        ...
        try:
            ...
            # Non-blocking: only sets cancel flags, workers clean up asynchronously
            logger.info("🛑 [PAUSE-ALL] Requesting non-blocking cancel on all workers...")
            cancelled = self.worker_pool.cancel_all_non_blocking()
            logger.info(f"✅ [PAUSE-ALL] Cancel requested on {cancelled} worker(s)")
