...
class ChatApiClient:
    ...
    def _log_request(self, method: str, url: str, headers: dict | None) -> float:
        ...
        log.debug("[LocalAI] -> %s %s", method, masked_url)
    ...
    def _log_response(self, started: float, resp: requests.Response) -> None:
        ...
        log.debug("[LocalAI] <- status=%s elapsed_ms=%s", resp.status_code, elapsed_ms)
...
class ChatController(QObject):
    ...
    def handle_chat_response(self, resp_json: dict):
        ...
        log.debug("Chat response received (%d chars)", len(assistant_text))
        if assistant_text:
