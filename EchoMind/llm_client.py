...
USAGE — from anywhere in the app:
    from EchoMind.llm_client import gapgpt_chat
...
-----------------------
The GapGPT Bearer token is resolved entirely from EchoMind Settings:
...
def _resolve_gapgpt_key() -> tuple[str, str]:
    ...
    try:
        ...
        raise LLMNoKeyError(
            f"Could not resolve GapGPT key from EchoMind Settings: {exc}"
        ) from exc
    ...
    *,
    ...
    if resp.status_code == 401:
        raise LLMAuthError(
            ...
            "Update your credential in Settings → EchoMind."
        )
