...
def _log_request(tag: str, method: str, url: str, headers: dict | None) -> float:
    masked_url = _mask_url(url)
    masked_headers = _mask_headers(headers)
    print(f"[GapGPT] {tag} -> {method} {masked_url} headers={masked_headers}")
    return time.perf_counter()

def _log_response(tag: str, started: float, resp: requests.Response) -> None:
    elapsed_ms = int((time.perf_counter() - started) * 1000)
    ctype = resp.headers.get("Content-Type")
    clen = resp.headers.get("Content-Length")
    print(f"[GapGPT] {tag} <- status={resp.status_code} elapsed_ms={elapsed_ms} content_type={ctype} content_length={clen}")

def _safe_parse_json(resp: requests.Response, *, tag: str) -> dict:
    ctype = (resp.headers.get("Content-Type") or "").lower()
    if "text/event-stream" in ctype:
        return _parse_sse_response(resp, tag=tag)
    try:
        return resp.json()
        ...
        try:
            ...
            body = ""
        snippet = body[:1200].replace("\r", " ").replace("\n", " ")
        print(f"[GapGPT] {tag} json decode failed: {type(e).__name__}: {e}")
        print(f"[GapGPT] {tag} content-type={resp.headers.get('Content-Type')} body_snippet={snippet!r}")
        raise Exception("GapGPT API returned non-JSON response.")

def _parse_sse_response(resp: requests.Response, *, tag: str) -> dict:
    try:
        raw = resp.text or ""
    except Exception:
        raw = ""
    ...
    if not content:
        raise Exception("GapGPT API returned SSE without content.")

    print(f"[GapGPT] {tag} parsed SSE content_len={len(content)}")
    return {
        "choices": [
            {
                "message": {
...
def reporter(
    ...
    m = Manage.instance()
    print("[GapGPT] standard_assist_search: resolving center/key")
    ...
    # ------------------------------------------------------
    print(
        f"[GapGPT] reporter model={payload.get('model')} center={_to_str(center)} key={_mask_key(api_key)}"
    )
    started = _log_request("reporter", "POST", url, headers)
    response = requests.post(url, headers=headers, json=payload)
    _log_response("reporter", started, response)
    result = _safe_parse_json(response, tag="reporter")
...
def chat(
    ...
    # ------------------------------------------------------
    m = Manage.instance()
    print("[GapGPT] standardize: resolving center/key")
    center, api_key = m.get_center_and_gapgpt_key()
...
"""
    ...
    # ------------------------------------------------------
    print(
        f"[GapGPT] chat model={payload.get('model')} center={_to_str(center)} key={_mask_key(api_key)}"
    )
    started = _log_request("chat", "POST", url, headers)
    response = requests.post(url, headers=headers, json=payload)
    _log_response("chat", started, response)
    result = _safe_parse_json(response, tag="chat")
    if response.status_code != 200:
        raise Exception(f"GapGPT API Error {response.status_code}: {result}")
...
def ImageQualityAnalyzer(
    ...
    print(
        f"[GapGPT] image_quality model={payload.get('model')} center={_to_str(center)} key={_mask_key(api_key)}"
    )
    started = _log_request("image_quality", "POST", url, headers)
    response = requests.post(url, headers=headers, json=payload)
    _log_response("image_quality", started, response)
    result = _safe_parse_json(response, tag="image_quality")
...
def BreastExpertAssistant(
    ...
    print(
        f"[GapGPT] breast_assistant model={payload.get('model')} center={_to_str(center)} key={_mask_key(api_key)}"
    )
    started = _log_request("breast_assistant", "POST", url, headers)
    response = requests.post(url, headers=headers, json=payload)
    _log_response("breast_assistant", started, response)
    result = _safe_parse_json(response, tag="breast_assistant")
...
"""
    ...
    url = "https://api.gapgpt.app/v1/chat/completions"
    print(
        f"[GapGPT] standard_assist_search model={payload.get('model')} center={_to_str(center)} key={_mask_key(api_key)}"
    )
    started = _log_request("standard_assist_search", "POST", url, headers)
    try:
        response = requests.post(url, headers=headers, json=payload)
    except Exception as e:
        print(f"[GapGPT] standard_assist_search request failed: {type(e).__name__}: {e}")
        raise
    _log_response("standard_assist_search", started, response)
    result = _safe_parse_json(response, tag="standard_assist_search")
...
def translate_report(
    ...
    # ------------------------------------------------------
    print(
        f"[GapGPT] standard_assist_search -> url={url} model={payload.get('model')} center={_to_str(center)} key={_mask_key(api_key)}"
    )
    response = requests.post(url, headers=headers, json=payload)
    print(f"[GapGPT] standard_assist_search <- status={response.status_code}")
    result = _safe_parse_json(response, tag="standard_assist_search")
    if response.status_code != 200:
        raise Exception(f"GapGPT API Error {response.status_code}: {result}")
...
def standard_assist_search(
    ...
    # ------------------------------------------------------
    print(
        f"[GapGPT] standardize -> url={url} model={payload.get('model')} center={_to_str(center)} key={_mask_key(api_key)}"
    )
    response = requests.post(url, headers=headers, json=payload)
    print(f"[GapGPT] standardize <- status={response.status_code}")
    result = _safe_parse_json(response, tag="standardize")
    if response.status_code != 200:
        raise Exception(f"GapGPT API Error {response.status_code}: {result}")
...
def standardize(user_msg: str,CENTER_Key: Optional[str] = None,model: str = "gpt-4.1-mini"):
    ...
    # ------------------------------------------------------
    print(
        f"[GapGPT] standardize model={payload.get('model')} center={_to_str(center)} key={_mask_key(api_key)}"
    )
    started = _log_request("standardize", "POST", url, headers)
    try:
        response = requests.post(url, headers=headers, json=payload)
    except Exception as e:
        print(f"[GapGPT] standardize request failed: {type(e).__name__}: {e}")
        raise
    _log_response("standardize", started, response)
    result = _safe_parse_json(response, tag="standardize")
    if response.status_code != 200:
        raise Exception(f"GapGPT API Error {response.status_code}: {result}")
