    ...
    def get_context_for_llm(self, max_cycles: int = 5) -> str:
        ...
        try:
            ...
            return (
                ...
                f"Memory #{self._memory_number} | {self._cycle_count} cycle(s) recorded.\n"
                f"Use this context to resolve patient references such as\n"
                f"'the 5th patient', 'yesterday's list', 'that patient',\n"
                f"'download those', or any continuation of a previous command.\n\n"
                f"{content}\n"
                f"=== END MEMORY ==="
            )
        except Exception:
