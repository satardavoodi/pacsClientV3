...
def _build_args(spec_module) -> list[str]:
    ...
    if not entry:
        raise ValueError("ENTRY_SCRIPT is required in EchoMind.nuitka.spec")
...
def main() -> int:
    ...
    spec_path = echomind_dir / "EchoMind.nuitka.spec"
    if not spec_path.exists():
        print(f"EchoMind.nuitka.spec not found: {spec_path}")
        return 2
