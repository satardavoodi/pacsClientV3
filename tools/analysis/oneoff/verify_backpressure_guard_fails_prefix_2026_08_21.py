"""One-off: prove tests/code/ui_services/test_list_stream_backpressure.py FAILS pre-fix.

Swaps the two touched source files for their HEAD versions, runs the new guard
file, then restores the working-tree versions in a finally block and verifies
...
TEST = "tests/code/ui_services/test_list_stream_backpressure.py"
...
def main():
    ...
    try:
        ...
        tail = [ln for ln in proc.stdout.splitlines() if ln.strip()][-30:]
        print("\n".join(tail))
    finally:
        for rel in RELS:
