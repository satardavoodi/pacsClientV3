...
class VTKWidget(QVTKRenderWindowInteractor):
    def __init__(self, parent=None, height_viewer=480, patient_widget=None):
        ...
        # v2.2.3.2.5: Disable multisampling ظ¤ VTK defaults to 8x MSAA.
        ...
        # os.getenv is slow on Windows (~3-5ms per call); calling it 2├ù per
        ...
        #   1. Extend timer 500ظْ2000ms.  All observed scroll gaps are <2s,
        #      so the timer never fires mid-session.  GC only re-enables
        #      when the user truly stops scrolling for 2 full seconds.
        #   2. Do NOT restore original thresholds on re-enable ظ¤ keep
    ...
    def _reenable_gc(self):
        ...
        v2.2.3.3.2: Keep elevated thresholds (700,50,50) ظ¤ do NOT restore
        ...
        if self._gc_suppressed:
            self._gc_suppressed = False
            # Keep thresholds at (700,50,50) ظ¤ gen-1 only runs every 50th
    ...
    def _do_render(self):
        ...
        try:
            ...
            logger.debug("[RENDER] ظû╢ Starting batched render")
            ...
            if hasattr(self.image_viewer, 'vtk_image_data') and self.image_viewer.vtk_image_data:
                ...
                if dims[0] == 0 or dims[1] == 0:
                    logger.warning(f"[RENDER] ظأب INCOMPLETE - Image has zero dimensions: {dims}")
                else:
                    logger.debug(f"[RENDER] ظ£ô Complete - dims: {dims[0]}x{dims[1]}x{dims[2]}")
            
        except Exception as e:
            logger.error(f"[RENDER] ظ£ù FAILED - Error: {e}")
            import traceback
    ...
    def _apply_sync_point(self, world_pos):
        ...
        logger.debug(
            "[SYNC SOURCE] viewer=%s orient=%d slice=%d ظْ world_pos=(%.2f, %.2f, %.2f)",
    ...
    def set_new_interactorstyle(self, style):
        ...
        if self.image_viewer is None:
            print("ظأبي╕ Cannot set interactor style - viewer not yet initialized")
            return
    ...
    def _capture_camera_state(self):
        try:
            ...
            # ظ£à Update protected scale when capturing state
    ...
    def _restore_camera_state(self, state):
        ...
        try:
            ...
            if camera:
                ...
                # ظ£à Update protected scale when restoring state
                self._protected_parallel_scale = state['parallel_scale']
    ...
    def start_process_series(self, vtk_image_data, metadata, series_index, id_vtk_widget, metadata_fixed):
        ...
        logger.info(f"[SERIES INIT] ظû╢ START - Series #{series_number} [{modality}] '{series_desc}'")
        ...
        try:
            ...
            if self.image_viewer and self.image_viewer.renderer:
                ...
                if camera:
                    parallel_scale = camera.GetParallelScale()
                    logger.info(f"[SERIES INIT] ظ£ô COMPLETE - Final parallel scale: {parallel_scale:.2f}")

        except Exception as e:
            logger.error(f"[SERIES INIT] ظ£ù FAILED - Error: {e}")
            import traceback
            logger.error(traceback.format_exc())
            raise
        finally:
    ...
    def reset_image(self, vtk_image_data, metadata):  # reload image
        ...
        logger.info(f"[IMAGE RESET] ظû╢ START - Series #{series_number} [{modality}] '{series_desc}'")
        ...
        try:
            # ظ£à Save current camera scale before reset
            saved_scale = None
            try:
                if self.image_viewer and self.image_viewer.renderer:
                    camera = self.image_viewer.renderer.GetActiveCamera()
            ...
            # ظ£à Always use zoom_to_fit to ensure image fills the viewer properly
            ...
            self.image_viewer.Render()
            logger.info(f"[IMAGE RESET] ظ£ô COMPLETE")

        except Exception as e:
            logger.error(f"[IMAGE RESET] ظ£ù FAILED - Error: {e}")
            import traceback
            logger.error(traceback.format_exc())
            raise
        finally:
    ...
    # v2.2.3.1.0: Removed switch_series_backup() ظ¤ dead code, superseded by switch_series().
    ...
    def switch_series(self, vtk_image_data, metadata, series_index, vtk_image_data_2=None, metadata_2=None,
                      metadata_fixed=None):
        """
        ظأة HIGHLY OPTIMIZED: Series switch with minimal flickering
        ...
        logger.info(f"[SERIES SWITCH] ظû╢ START - Series #{series_number} [{modality}] '{series_desc}'")
        ...
        if self.last_series_show == series_index:
            logger.info(f"[SERIES SWITCH] ظص SKIP - Already showing series {series_index}")
            return False
        ...
        # ≡اش SHOW SPINNER WITH SMART MESSAGE BASED ON SERIES SIZE
        ...
        try:
                ...
                try:
                    ...
                    if is_combined_new != is_combined_current:
                        ...
                        if is_combined_new:
                            ...
                            # ظأة FAST PATH: Just update image data without full viewer recreation
                            ...
                            # ظ£à CRITICAL: Update _protected_parallel_scale to match the 
                            ...
                            try:
                                ...
                                logger.info(f"[SERIES SWITCH] ظ£ô COMPLETE (FAST) - Final scale: {final_scale:.2f}")
                            except:
                                logger.info(f"[SERIES SWITCH] ظ£ô COMPLETE (FAST)")
            ...
            # ظأة BATCHED CREATION: All operations grouped together
            ...
            # ظأة SINGLE BATCHED RENDER at the end (not multiple renders)
            ...
            try:
                camera = self.image_viewer.renderer.GetActiveCamera()
                final_scale = camera.GetParallelScale() if camera else 0
                logger.info(f"[SERIES SWITCH] ظ£ô COMPLETE (SLOW) - Final scale: {final_scale:.2f}")
            except:
                logger.info(f"[SERIES SWITCH] ظ£ô COMPLETE (SLOW)")
            
        except Exception as e:
            logger.error(f"[SERIES SWITCH] ظ£ù FAILED - Error: {e}")
            import traceback
            logger.error(traceback.format_exc())
            raise
    ...
    def _get_smart_spinner_message(self, vtk_image_data, metadata):
        ...
        try:
            ...
            if vtk_image_data:
                ...
                if num_slices > 200:
                    return f"≡اôè Loading large series... ({num_slices} images)"
                elif num_slices > 100:
                    return f"≡اô╖ Switching series... ({num_slices} images)"
                elif num_slices > 50:
    ...
    def set_slice(self, slice_index):
        ...
        # ظ£à CRITICAL: Save current camera zoom before slice change
        # v2.2.3.4.0: Skip during wheel scroll ظ¤ the wheel event is consumed
        # (event.accept) so VTK's built-in zoom is blocked.  Camera save/
        # restore costs ~3-5ms per frame (VTK ظْ Python round-trips + comparison).
        ...
        # ظ£à CRITICAL: Force restore camera zoom after slice change
        ...
        # floating-point drift.  Tolerance widened from 0.001 ظْ 0.05 so minor
        # per-frame FP jitter in SetSlice() no longer fires a second Render()
        # on every scroll (was measured as 60ظô80ms extra per scroll in Mode B).
            ...
            try:
                ...
                if saved_scale is not None and camera:
                    ...
                    if abs(current_scale - _ref_scale) > 0.05:
                        logger.warning(f"[set_slice] Zoom change detected! scale={current_scale:.4f} ظْ reverting to {_ref_scale:.4f}")
        ...
        # v2.2.3.4.0: Skip during wheel scroll ظ¤ ruler tools are not
        ...
        # Lock Sync callback ظ¤ fires on EVERY slice change regardless of source
    ...
    def wheelEvent(self, event):
        ...
        """
        # ظ£à ALWAYS log to confirm this method is being called
        t_event_receive = now_ms()
        self._last_scroll_event_ms = t_event_receive
        # v2.2.3.3.2: Suppress GC during scroll burst.
        # Save original thresholds only once ظ¤ if we already have saved
        ...
        if not self._gc_suppressed:
            ...
            gc.set_threshold(700, 50, 50)  # 5├ù less frequent gen-1/gen-2
            if gc.isenabled():
                gc.disable()
            self._gc_suppressed = True
        # v2.2.3.3.9: Tighten throttle from 500msظْ250ms so the busy flag
        ...
        try:
            ...
            if not self._wheel_coalesce_timer.isActive():
                if _since_last >= self._adaptive_frame_gap_ms:
                    # Enough time since last render ظْ render immediately (0ms latency)
                    self._flush_pending_wheel_slice()
                else:
                    # Within adaptive gap ظْ schedule for remaining time
            ...
            # 3-8ms per event ├ù 3-5 queued events = 9-40ms per frame cycle.

            # ظ£à CRITICAL: CONSUME the event - DO NOT let parent handle it
            event.accept()
            
        except Exception as e:
            # ظ£à Even on error, CONSUME the event to prevent VTK zoom fallback
            ...
            if hasattr(self.image_viewer, 'curved_mpr_mode') and self.image_viewer.curved_mpr_mode:
                ...
                if key == Qt.Key_G and modifiers == Qt.NoModifier:
                    ...
                    if point_count >= 2:
                        ...
                        print(f"ظ£ô Curved MPR generated with {point_count} points")
                    else:
                        print(f"ظأبي╕ Need at least 2 points (have {point_count})")
                    event.accept()
                    return
                ...
                # ESC key: Exit curved MPR mode
                    ...
                    print("ظ£ô Curved MPR mode deactivated")
                    event.accept()
                    return
    ...
    def dropEvent(self, event):
        ...
        try:
            ...
            # ≡اش Show loading spinner immediately when series is dropped
