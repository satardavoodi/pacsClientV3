...
class DownloadManagerWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # ✅ PHASE 1.1: Clean Startup - Don't auto-load downloads
        # Only run cleanup to remove old incomplete downloads (>7 days)
        # UI starts completely empty for fresh session
        QTimer.singleShot(0, lambda: self._cleanup_old_incomplete_downloads(max_age_days=7))
        logger.info("🎯 Download Manager starting with CLEAN UI (no auto-load)")
        ...
        # ✅ PHASE 1.1 FIX: Don't auto-load database progress on startup
        # This would load incomplete downloads, defeating the clean UI goal
        # self._init_database_progress()  # ← DISABLED for clean startup
    ...
    def _init_database_progress(self):
        ...
        try:
            ...
            if incomplete_downloads:
                ...
                for progress in incomplete_downloads:
                    ...
                    if not any(d.study_uid == study_uid for d in self.study_downloads):
                        ...
                        # ✅ FIX: Don't add directly - will be added by _refresh_table_order() at the end
            ...
            # ✅ FIX: Refresh table with proper priority grouping after loading all items
            if incomplete_downloads:
    ...
    def _immediate_preemption_check(self, high_priority_study):
        ...
        ✅ CRITICAL PRIORITY RULE: Pauses ALL other downloads
        """
        try:
            ...
            # ✅ CRITICAL PRIORITY RULE: Pause ALL other downloads
            if high_priority_study.priority == "Critical":
                print(f"   🚨 CRITICAL PRIORITY - pausing ALL other downloads")
                patient_name = (high_priority_study.patient_name or "Unknown")[:30]
                self.log_message(f"🚨 CRITICAL PRIORITY: Pausing ALL other downloads for {patient_name}")
                
                # Pause all currently downloading studies
                paused_count = 0
                for study_download in self.study_downloads:
                    if (study_download.study_uid != high_priority_study.study_uid and 
                        study_download.status == "Downloading"):
                        self._fast_pause_download(study_download)
                        paused_count += 1
                
                if paused_count > 0:
                    print(f"   ⏸️ Paused {paused_count} download(s)")
                    self.log_message(f"⏸️ Paused {paused_count} download(s) for Critical priority")
                
                # Ensure critical study is ready to start
                if high_priority_study.status not in ["Downloading", "Completed"]:
                    high_priority_study.status = "Pending"
                
                # Start the critical priority download immediately
                QTimer.singleShot(10, lambda: self.start_study_download_item(high_priority_study))
                return
            
            # For non-Critical priorities: standard single-download preemption
            ...
            current_name = (current_download.patient_name or "Unknown")[:20]
            print(f"   📊 Current download: {current_name} [{current_download.priority}] (val={current_priority_value}, order={current_open_order})")
            ...
            if should_preempt:
                ...
                high_name = (high_priority_study.patient_name or "Unknown")[:20]
                current_name = (current_download.patient_name or "Unknown")[:20]
                self.log_message(f"⚡ PREEMPT [{reason}]: [{high_priority_study.priority}] {high_name} → [{current_download.priority}] {current_name}")
    ...
    def _try_auto_add_study(self, study_uid: str, priority_name: str):
        ...
        try:
            ...
            if patient_info:
                ...
                # ✅ FIX: Use _refresh_table_order() to ensure proper priority grouping
                # Don't add directly to table as it bypasses grouping logic!
    ...
    def _cleanup_old_incomplete_downloads(self, max_age_days=7):
        """
        Clean up incomplete downloads older than specified days.
        
        This prevents the system from accumulating old incomplete downloads
        that are no longer useful to the user.
        
        Args:
            max_age_days: Maximum age in days for incomplete downloads (default: 7)
        
        Only removes INCOMPLETE downloads (Paused, Failed, Pending, Downloading).
        Completed downloads are kept permanently for duplicate detection.
        """
        logger.info("=" * 80)
        logger.info(f"🧹 CLEANING UP INCOMPLETE DOWNLOADS OLDER THAN {max_age_days} DAYS...")
        logger.info("=" * 80)
        
        try:
            from PacsClient.utils.database import get_all_download_progress, delete_download_progress
            ...
            # Find old incomplete downloads
            old_incomplete = []
            for record in progress_records:
                status = record.get('status', '')
                
                # Skip completed downloads (keep forever for duplicate detection)
                if status == 'Completed':
                    continue
                
                # Get timestamp (prefer created_at, fallback to last_update)
                timestamp = record.get('created_at') or record.get('last_update')
                
                if not timestamp:
                    # No timestamp at all - consider it old and clean it up
                    old_incomplete.append(record)
                    continue
                ...
                if isinstance(timestamp, str):
                    try:
                        ...
                        try:
                            ...
                            # Failed to parse - consider it old
                            old_incomplete.append(record)
                            continue
            ...
            # Log what we found
            logger.info(f"✅ Found {len(old_incomplete)} old incomplete downloads to clean up")
            
            # Clean up each old incomplete download
            cleaned_count = 0
            for record in old_incomplete:
                study_uid = record.get('study_uid')
                patient_name = record.get('patient_name', 'Unknown')
                status = record.get('status', 'Unknown')
                
                logger.info(f"   🗑️  Removing: {patient_name[:30]} ({status}) - {study_uid[:30]}...")
                ...
                try:
                    ...
                    logger.warning(f"   ⚠️  Failed to delete {study_uid}: {e}")
            
            logger.info(f"✅ Cleaned up {cleaned_count} old incomplete downloads")
            logger.info("=" * 80)
    ...
    def _validate_study_with_server(self, study_uid, db_record, study_data):
        """
        ✅ PHASE 3.1: Server Validation Method
        
        Validate study against server and determine action.
        Compares server state with database state to detect changes.
        
        Args:
            study_uid: The study UID to validate
            db_record: Database record with download progress
            study_data: Study data from patient selection (contains server info)
        
        Returns:
            {
                'action': 'skip' | 'resume' | 'incremental' | 'restart',
                'changes': {
                    'new_series': [],
                    'new_images_count': 0,
                    'removed_series': [],
                    'total_server_images': 0,
                    'total_db_images': 0
                },
        ...
        """
        try:
            logger.info(f"🔍 Validating study with server: {study_uid[:30]}...")
            
            # 1. Get current state from server
            from PacsClient.components.grpc_client import DicomGrpcClient
            
            socket_config = get_socket_config()
            server_address = socket_config.get_socket_host()
            grpc_client = DicomGrpcClient(host=server_address, port=50051, timeout=30.0)
            
            # Fetch series list from server
            result = grpc_client.get_study_thumbnails_with_timeout(study_uid, timeout=30.0)
            
            if not result or not result.get('series_list'):
                logger.warning(f"⚠️ Could not fetch series info from server for {study_uid[:30]}")
                return {
                    'action': 'restart',
                    'changes': {},
                    'message': 'Could not validate with server. Will restart download.'
                }
            
            server_series_list = result['series_list']
            server_series_dict = {s.get('series_number'): s.get('image_count', 0) for s in server_series_list}
            total_server_images = sum(server_series_dict.values())
            
            logger.info(f"📊 Server state: {len(server_series_dict)} series, {total_server_images} images")
            
            # 2. Get database state
            db_status = db_record.get('status', 'Unknown')
            db_downloaded = db_record.get('downloaded_count', 0)
            db_total = db_record.get('total_instances', 0)
            
            logger.info(f"💾 Database state: Status={db_status}, {db_downloaded}/{db_total} images")
            
            # 3. Compare states and determine action
            
            # Case 1: Completed and unchanged
            if db_status == 'Completed' and db_total == total_server_images:
                logger.info(f"✅ Study unchanged and complete - SKIP")
                return {
                    'action': 'skip',
                    'changes': {
                        'total_server_images': total_server_images,
                        'total_db_images': db_total
                    },
                    'message': f'Study already downloaded ({db_total} images)'
                }
            
            # Case 2: Completed but server has more images (incremental)
            if db_status == 'Completed' and total_server_images > db_total:
                new_images = total_server_images - db_total
                logger.info(f"📥 Study has new data: {new_images} new images - INCREMENTAL")
                return {
                    'action': 'incremental',
                    'changes': {
                        'new_series': [],  # TODO: Detailed series comparison
                        'new_images_count': new_images,
                        'removed_series': [],
                        'total_server_images': total_server_images,
                        'total_db_images': db_total
                    },
                    'message': f'New data detected: {new_images} new images'
                }
            
            # Case 3: Incomplete and unchanged (resume)
            if db_status in ['Paused', 'Pending', 'Downloading', 'Failed'] and db_total == total_server_images:
                progress_pct = int((db_downloaded / db_total * 100)) if db_total > 0 else 0
                logger.info(f"⏸️ Study incomplete but unchanged - RESUME from {progress_pct}%")
                return {
                    'action': 'resume',
                    'changes': {
                        'total_server_images': total_server_images,
                    ...
                    'message': f'Resume download from {progress_pct}% ({db_downloaded}/{db_total} images)'
                }
            
            # Case 4: Structure changed significantly (restart)
            if db_total != total_server_images:
                logger.info(f"⚠️ Study structure changed significantly - RESTART")
                return {
                    'action': 'restart',
                    'changes': {
                        'total_server_images': total_server_images,
                    ...
                    'message': f'Study structure changed (was {db_total}, now {total_server_images} images)'
                }
            
            # Default: Resume (safest option)
            logger.info(f"❓ Unclear state - defaulting to RESUME")
            return {
                'action': 'resume',
                'changes': {
                    'total_server_images': total_server_images,
            ...
            logger.error(f"❌ Error validating study with server: {e}")
            import traceback
            logger.error(traceback.format_exc())
            
            # On error, allow restart
            return {
                'action': 'restart',
                'changes': {},
    ...
    def _load_persisted_state(self):
        """
        Load INCOMPLETE downloads from database to resume interrupted downloads.
        
        This ensures:
        - Active/paused downloads can be resumed after app restart
        - Completed downloads stay in database for "already downloaded" checks
        - UI starts clean without showing old completed downloads
        - Old incomplete downloads are cleaned up automatically
        
        Note: Completed downloads remain in database to prevent re-downloading,
        but are NOT displayed in the UI to keep it clean.
        """
        # First, clean up old incomplete downloads (default: 7 days)
        self._cleanup_old_incomplete_downloads(max_age_days=7)
        
        logger.info("=" * 80)
        logger.info("💾 LOADING INCOMPLETE DOWNLOADS FROM DATABASE...")
        logger.info("=" * 80)
        ...
        try:
            ...
            # ✅ FIX: Only load INCOMPLETE downloads (not Completed)
            # This keeps the UI clean while database still has history for duplicate checks
            incomplete_records = [r for r in progress_records if r.get('status') != 'Completed']
            
            completed_count = len([r for r in progress_records if r.get('status') == 'Completed'])
            logger.info(f"✅ Found {len(incomplete_records)} INCOMPLETE downloads to restore")
            logger.info(f"ℹ️  Found {completed_count} completed downloads (kept in database, not loaded to UI)")
            
            # Log each incomplete download for debugging
            for record in incomplete_records:
                logger.info(f"   📋 Restoring: {record.get('study_uid')} - {record.get('patient_name')} ({record.get('status')})")
            ...
            for record in incomplete_records:
                ...
                total_count = record.get('total_instances', 1)  # Avoid division by zero
                progress_percent = int((downloaded_count / total_count) * 100) if total_count > 0 else 0
                ...
                study_download = StudyDownloadItem(
                    ...
                    status=record.get('status', 'Paused')  # Restore original status
                )
                
                # Set progress information from database
                study_download.progress = progress_percent
                study_download.downloaded_images = downloaded_count
                study_download.image_count = total_count
                study_download.downloaded_series = 0  # Not in database
                study_download.series_count = 0  # Not in database
                study_download.priority = record.get('priority', 'Normal')  # Restore priority if available
            ...
            logger.info(f"✅ Successfully loaded {len(incomplete_records)} incomplete downloads into memory")
            logger.info(f"📊 Total study_downloads in memory: {len(self.study_downloads)}")
            logger.info("=" * 80)
            
        except Exception as e:
            logger.error(f"❌ Error loading incomplete downloads from database: {e}")
            import traceback
            logger.error(traceback.format_exc())
    ...
    def _refresh_ui_from_persisted_state(self):
        ...
        try:
            # ✅ FIX: Use _refresh_table_order() to ensure proper priority grouping
            # Don't call add_study_download_to_table() in loop - it bypasses grouping
            self._refresh_table_order()
            ...
            for idx, d in enumerate(self.study_downloads, 1):
                patient_name = (d.patient_name or "Unknown")[:20]
                self.log_message(f"   {idx}. {patient_name} - Status: {d.status}, Priority: {d.priority}")
            # === END DIAGNOSTIC ===
    ...
    def _refresh_table_order(self):
        ...
        try:
            logger.info(f"🔄 ========== REFRESH TABLE START (items: {len(self.study_downloads)}) ==========")
            ...
            # ✅ DIAGNOSTIC: Log each download's priority as we group them
            for download in self.study_downloads:
                priority = download.priority
                patient_short = (download.patient_name or "Unknown")[:30]
                logger.info(f"🔍 GROUPING: {patient_short} → Priority={priority} (type: {type(priority).__name__})")
                
                if priority in priority_groups:
                    priority_groups[priority].append(download)
                    logger.debug(f"   ✅ Added to {priority} group")
                else:
                    logger.warning(f"   ⚠️ Unknown priority '{priority}', adding to Normal group")
                    priority_groups["Normal"].append(download)
            
            # ✅ FIX: Sort each group by created_at (NEWEST FIRST) so active downloads appear at top
            for priority in priority_groups:
                priority_groups[priority].sort(key=lambda x: x.created_at, reverse=True)
            ...
            for priority in ["Critical", "High", "Normal", "Low"]:
                ...
                logger.info(f"📦 Processing {priority} group: {len(group_items)} items")
                for item in group_items:
                    logger.info(f"   - {item.patient_name[:30]}: priority={item.priority}")
                
                # Skip empty groups if not showing them
                if not group_items and not self._show_empty_groups:
                    logger.info(f"   ⏭️ Skipping empty {priority} group")
                    continue
                ...
                if priority not in self._collapsed_groups:
                    ...
                    for download in visible_items:
                        # ✅ FIX: Build row directly here instead of calling add_study_download_to_table()
                        # This ensures items are added in the correct priority group order
                        self._build_table_row_for_download(download)
            ...
            logger.info(f"🔄 ========== REFRESH TABLE END (rows created: {final_row_count}) ==========")
    ...
    def refresh_progress_from_database(self):
        ...
        try:
            ...
            # ✅ FIX: Refresh table order once after all items added (not in loop)
            # This ensures proper priority grouping
            self._refresh_table_order()
    ...
    def _start_next_pending_download(self):
        ...
        try:
            ...
            if selected_download:
                ...
                patient_name = (selected_download.patient_name or "Unknown")[:30]
                self.log_message(f"{action} [{selected_download.priority}]: {patient_name}")
    ...
    def _try_auto_retry_failed(self):
        ...
        for sd in self.study_downloads:
            if (sd.status == "Failed" and 
                ...
                patient_name = (sd.patient_name or "Unknown")[:30]
                self.log_message(f"🔄 Auto-retrying: {patient_name}")
                sd.status = "Pending"
    ...
    def _instant_add_to_table(self, study_download):
        ...
        try:
            ...
            if existing_row >= 0:
                ...
                # ✅ FIX: Use _refresh_table_order() to ensure proper priority grouping
                # Don't call add_study_download_to_table() directly - it bypasses grouping
                self._refresh_table_order()
        ...
        try:
            ...
            if active_count < self.MAX_CONCURRENT_DOWNLOADS:
                ...
                if next_candidate and next_candidate.status in ["Pending", "Paused"]:
                    patient_name = (next_candidate.patient_name or "Unknown")[:30]
                    self.log_message(f"⚡ Auto-starting [{next_candidate.priority}]: {patient_name}")
    ...
    def _get_currently_downloading(self):
        ...
        """
        # ✅ FIX: Handle None patient_name gracefully
        statuses = [((sd.patient_name or "Unknown")[:15], sd.status) for sd in self.study_downloads[:5]]
            ...
            if active_study_uids:
                for study_download in self.study_downloads:
                    if study_download.study_uid in active_study_uids:
                        patient_name = (study_download.patient_name or "Unknown")[:20]
                        print(f"   ✅ [GET-CURRENT] Found via active_workers: {patient_name} [{study_download.status}]")
                        return study_download
        ...
        for study_download in self.study_downloads:
            if study_download.status == "Downloading":
                patient_name = (study_download.patient_name or "Unknown")[:20]
                print(f"   ✅ [GET-CURRENT] Found via status: {patient_name}")
                return study_download
    ...
    def _check_and_preempt_for_priority(self, high_priority_study):
        ...
        - If a Critical priority item: PAUSE ALL other downloads
        - If a High priority item: Pause lower priority downloads
        ...
        try:
            ...
            if high_priority_study.priority == "Critical":
                ...
                # Mark the critical study as Pending so it can be started
            ...
            if high_priority_value < current_priority_value:
                ...
                high_name = (high_priority_study.patient_name or "Unknown")[:20]
                current_name = (current_download.patient_name or "Unknown")[:20]
                self.log_message(f"⚡ PREEMPT: [{high_priority_study.priority}] {high_name} → [{current_download.priority}] {current_name}")
    ...
    def update_study_priority(self, study_uid: str, new_priority: str, trigger_preemption: bool = True):
        ...
        try:
            ...
            if new_value != old_value:
                patient_name = (study_download.patient_name or "Unknown")[:25]
                self.log_message(f"📊 Priority: {patient_name} [{old_priority}] → [{new_priority}]")
            
            # Update UI immediately (non-blocking)
            if self.isVisible():
                QTimer.singleShot(0, lambda sd=study_download: self.update_study_table_row(sd))
            
            # ✅ FIX: Refresh table to update priority grouping
            # Ensures the item moves to the correct visual priority group
            if self.isVisible():
        ...
        try:
            ...
            for i, progress in enumerate(incomplete_downloads):
                ...
                if existing_download:
                    ...
                    # Add to list (table will be refreshed at the end)
                    self.study_downloads.append(study_download)
                    # ✅ FIX: Don't add directly - will be added by _refresh_table_order() at the end
            ...
            # ✅ FIX: Refresh table with proper priority grouping
            self._refresh_table_order()
    ...
    def clear_completed(self):
        ...
        # Also add regular downloads (if any)
        for download in self.downloads:
            self.add_download_to_table(download)
    ...
    def force_clear_all(self):
        ...
        # ✅ PHASE 2.1: Updated Reset Dialog - Clarify database preservation
        reply = QMessageBox.warning(
            self,
            "Reset Download Manager",
            f"⚠️ This will RESET the Download Manager:\n\n"
            f"• {table_row_count} rows in UI table will be cleared\n"
            f"• {internal_count} items in active queues will be removed\n"
            f"• All active downloads will be stopped\n"
            f"• UI will be reset to empty state\n\n"
            f"✅ Download history in database will be PRESERVED\n"
            f"(You can resume downloads later by re-selecting patients)\n\n"
            f"This behaves exactly like closing and reopening the app.\n\n"
        ...
        try:
            ...
            # ✅ PHASE 2.1: Database Preservation - Don't delete persistence/database
            # Database keeps all download history for smart resume logic
            self.log_message("   Step 5: Database preserved (download history intact)")
            
            # 5. Update status summary
            self.log_message("   Step 6: Updating status summary...")
            self.update_status_summary()
            
            # 6. Hide diagnostic button
            self.diagnostic_btn.hide()
            
            self.log_message("✅ RESET COMPLETE: UI cleared, database preserved")
            self.log_message("ℹ️ You can now add new downloads from the patient list")
            
            QMessageBox.information(
                self,
                "Reset Complete",
                "✅ Download Manager has been reset.\n\n"
                "• UI cleared\n"
                "• Active queues cleared\n"
                "• Database history preserved\n\n"
                "You can now add new downloads or resume previous ones."
            )
    ...
    def add_study_downloads(self, study_data_list, server_info=None, start_immediately=False):
        ...
        try:
            ...
            for study_data in study_data_list:
                ...
                # ✅ FIX: Check BOTH in-memory AND database for existing downloads
                existing_study = None
                
                # First check in-memory list (incomplete downloads loaded on startup)
                for sd in self.study_downloads:
                    if sd.study_uid == study_uid:
                        existing_study = sd
                        logger.info(f"✅ FOUND IN MEMORY: Status={sd.status}, Images={sd.downloaded_images}/{sd.image_count}")
                        break
                
                # ✅ PHASE 3.2: Enhanced database check with server validation
                if not existing_study:
                    logger.info(f"❌ NOT FOUND IN MEMORY - Checking database...")
                    try:
                        from PacsClient.utils.database import get_download_progress
                        db_record = get_download_progress(study_uid)
                        
                        if db_record:
                            # Found in database - validate with server
                            db_status = db_record.get('status', 'Unknown')
                            downloaded_count = db_record.get('downloaded_count', 0)
                            total_count = db_record.get('total_instances', 0)
                            logger.info(f"✅ FOUND IN DATABASE: Status={db_status}, Images={downloaded_count}/{total_count}")
                            
                            # Validate with server to determine action
                            validation = self._validate_study_with_server(study_uid, db_record, study_data)
                            action = validation['action']
                            message = validation['message']
                            
                            logger.info(f"🎯 Validation result: {action} - {message}")
                            
                            if action == 'skip':
                                # Study is complete and unchanged - skip
                                self.log_message(f"✅ {message}")
                                QMessageBox.information(
                                    self,
                                    "Already Downloaded",
                                    f"Study '{patient_name}' has already been downloaded.\n\n"
                                    f"{message}\n\n"
                                ...
                                # Study is incomplete - offer to resume
                                progress_pct = validation['changes'].get('progress_percent', 0)
                                reply = QMessageBox.question(
                                    self,
                                    "Resume Download",
                                    f"Study '{patient_name}' was partially downloaded ({progress_pct}% complete).\n\n"
                                    f"{message}\n\n"
                                    f"Do you want to resume from where it stopped?",
                                    QMessageBox.Yes | QMessageBox.No,
                                    QMessageBox.Yes
                                )
                                
                                if reply == QMessageBox.Yes:
                                    # Create StudyDownloadItem with resumed state
                                    self.log_message(f"⏸️ Resuming: {patient_name} from {progress_pct}%")
                                    # Will be handled below in the "new download" section
                                else:
                                    skipped_count += 1
                                    continue
                            
                            elif action == 'incremental':
                                # New data detected - offer incremental download
                                new_images = validation['changes'].get('new_images_count', 0)
                                reply = QMessageBox.question(
                                    self,
                                    "New Data Detected",
                                    f"Study '{patient_name}' has new data since last download:\n\n"
                                    f"• {new_images} new images detected\n\n"
                                    f"Download only the new data?",
                                    QMessageBox.Yes | QMessageBox.No,
                                    QMessageBox.Yes
                                )
                                
                                if reply == QMessageBox.Yes:
                                    self.log_message(f"📥 Incremental download: {patient_name} (+{new_images} images)")
                                    # Will be handled below in the "new download" section
                                else:
                                    skipped_count += 1
                                    continue
                            
                            # If we get here, user chose to proceed (resume/incremental/restart)
                            # Continue to add as new download below
                            
                        else:
                            logger.info(f"❌ NOT FOUND IN DATABASE - This is a NEW download")
                            
                    except Exception as e:
                        logger.warning(f"⚠️ Failed to check database: {e}")
                        logger.info(f"❌ Assuming NEW download due to database error")
                ...
                # ✅ FIX: Determine priority BEFORE creating download item
                initial_priority = "Normal"  # Default
                
                # Check priority from priority manager FIRST
                if PRIORITY_MANAGER_AVAILABLE:
                    try:
                        priority_manager = get_download_priority_manager()
                        ...
                        if priority_manager.is_tab_open(study_uid):
                            ...
                            if study_uid in priority_manager._viewer_series and priority_manager._viewer_series[study_uid]:
                                ...
                                logger.info(f"🚨 Detected CRITICAL priority for {patient_name} (viewer open)")
                            else:
                                initial_priority = "High"
                                logger.info(f"⬆️ Detected HIGH priority for {patient_name} (tab open)")
                    except Exception as e:
                        logger.debug(f"Priority manager check failed: {e}")
                        pass  # Priority manager is optional
                
                # Create study download item with validated data and correct priority
                study_download = StudyDownloadItem(
                    patient_id=patient_id,
                    patient_name=patient_name,
                    study_uid=study_uid,
                ...
                # Set priority immediately (before adding to list)
                study_download.priority = initial_priority
                logger.info(f"🏷️ SET PRIORITY: {patient_name[:30]} → {initial_priority} (study_uid: {study_uid[:30]})")
                ...
                # Debug: Log series list population
                logger.debug(f"📋 Batch download created with priority={initial_priority}, {len(study_download.series_list)} series for {study_download.patient_name}")
                
                # Set server info
                study_download.server_info = server_info
                
                # Add to list (will be sorted later) - priority is already set correctly
                self.study_downloads.append(study_download)
            ...
            if added_count > 0:
                ...
                # ✅ FIX: Force a second refresh after a short delay to ensure priority grouping is correct
                # This handles cases where priority is set asynchronously by priority manager
                QTimer.singleShot(100, self._refresh_table_order)
    ...
    def _build_table_row_for_download(self, study_download):
        """
        Build a table row for a download at the current position (end of table).
        Used by _refresh_table_order() to build the table in priority order.
        Does NOT check for existing rows - assumes table is being built from scratch.
        """
        try:
            row = self.download_table.rowCount()
            self.download_table.insertRow(row)
            logger.debug(f"   📍 Building row {row} for: {study_download.patient_name} (Priority: {study_download.priority})")
            
            # Populate the row with download info (same as add_study_download_to_table but without checks)
            self._populate_table_row(row, study_download)
            
        except Exception as e:
            logger.error(f"Error building table row: {e}")
            import traceback
            logger.error(traceback.format_exc())
    ...
    def add_study_download_to_table(self, study_download):
        ...
        try:
            ...
            # Populate the row with all columns
            self._populate_table_row(row, study_download)
            
        except Exception as e:
            logger.error(f"Error adding study download to table: {e}")
            import traceback
            logger.error(traceback.format_exc())
    
    def _populate_table_row(self, row: int, study_download):
        """Populate a table row with download information"""
        try:
            ...
            logger.debug(f"✅ Successfully populated row {row} for {study_download.patient_name}")
    ...
    def _on_priority_changed(self, study_download, priority_index, combo_widget):
        ...
        if old_priority != new_priority:
            ...
            # ✅ FIX: ALWAYS refresh table to update priority grouping
            # This ensures the item moves to the correct visual priority group
            QTimer.singleShot(100, self._refresh_table_order)
    ...
    def update_status_summary(self):
        ...
        if downloading > 0:
            ...
            if current:
                ...
                patient_name = (current.patient_name or "Unknown")[:15]
                status_parts.append(f"⬇️ {patient_name}... ({progress_text})")
            else:
