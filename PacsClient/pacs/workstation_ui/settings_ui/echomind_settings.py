...
class EchoMindSettingsWidget(QWidget):
    ...
    def _build_ui(self):
        ...
        stt_group = QGroupBox("EchoMind Secretary Voice-to-Text Provider")
        stt_layout = QFormLayout(stt_group)
        self.provider_combo = QComboBox()
        self.provider_combo.addItem("IranNevis API", userData="native")
        self.provider_combo.addItem("External Voice-to-Text (V2T)", userData="v2t")
        self.provider_combo.currentIndexChanged.connect(self._on_provider_changed)
        stt_layout.addRow("Provider:", self.provider_combo)
    ...
    def _authenticate_and_refresh(self, key: str, silent: bool):
        ...
        for model, mins in sorted(tr_models.items(), key=lambda kv: str(kv[0]).lower()):
            try:
                ...
                m = 0.0
            tr_lines.append(f"• {model}: {m:.1f} min")
