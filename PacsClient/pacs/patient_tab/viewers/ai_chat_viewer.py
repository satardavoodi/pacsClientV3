...
from EchoMind.settings_store import get_echomind_api_key, set_echomind_api_key
...
class EchoMindLoginPage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        subtitle = QLabel("Access is configured in Settings -> EchoMind.", self)
        ...
        self.key_input.setPlaceholderText("Managed in Settings -> EchoMind")
        ...
        self.btn_login = QPushButton("Reload Settings", self)
        ...
        self._prefill_key()
        QTimer.singleShot(0, self._auto_login_from_settings)

    def _prefill_key(self):
        saved_key = (get_echomind_api_key() or "").strip()
        if not saved_key:
            self.status_lbl.setText("No saved key. Open Settings -> EchoMind to set it.")
            return
        self.key_input.setText(saved_key)
        self.status_lbl.setText("Saved key detected. Verifying...")

    def _attempt_login(self):
        self._attempt_saved_login(silent=False)

    def _auto_login_from_settings(self) -> None:
        self._attempt_saved_login(silent=True)

    def _attempt_saved_login(self, *, silent: bool) -> bool:
        saved_key = (get_echomind_api_key() or "").strip()
        if not saved_key:
            if not silent:
                self.status_lbl.setText("No saved key. Open Settings -> EchoMind to set it.")
        ...
        ok, center, error = manager.validate_key(saved_key)
        if not ok:
            self.status_lbl.setText((error or "Invalid access key.") + " Update it in Settings -> EchoMind.")
            return False

        set_echomind_api_key(saved_key)
        try:
...
class AIChatViewer(QWidget):
    """Multi-mode AI surface: shows Login -> ModePicker -> locked OneChatPage."""
