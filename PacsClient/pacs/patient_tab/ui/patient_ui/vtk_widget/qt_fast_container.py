...
class QtFastContainer(QWidget):
    ...
    def _start_qt_viewer(self, metadata, metadata_fixed):
        ...
        # is unknown, the render proceeds (identical to legacy). Made unconditional
        # 2026-07-05 (flag AIPACS_VIEWPORT_STUDY_IDENTITY_GATE retired; the gate is a
        # fail-open safety add that never blocks a correct render, only a stale/wrong one).
        try:
            ...
            # PRIMARY signal: SeriesInstanceUID mismatch. A viewport must show EXACTLY
            # the series last requested for it (change_series stamps the intended
            # series_uid); a re-render carrying a different series_uid is a
            # superseded/stale stomp. series_uid is globally unique and - unlike
            # study_uid - cannot be corrupted by a series DOWNLOADED and SAVED IN THE DB
            # under the wrong study (48912: the previous exam's series 1000004 carries
            # the CURRENT study_uid in its metadata, so the study signals falsely
            # matched; the series_uid still exposes the mismatch). Each viewport gets
            # its OWN intended stamp, so a paired-MG side-by-side (separate viewport,
            # its own change_series) is not affected - within ONE viewport the rendered
            # series_uid always equals the intended one for a legit switch/grow/reset.
            # Secondary signal: cross-STUDY mismatch (defence in depth). Both fail open
            # when their value is unknown.
            ...
            # Always-on decision trace so a non-block is diagnosable.
            try:
                logger.info(
