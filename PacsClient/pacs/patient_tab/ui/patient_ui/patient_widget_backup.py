...
from PySide6.QtWidgets import QHBoxLayout, QSlider, QLabel, QScrollArea, QGridLayout, QToolBar, QPushButton, \
    QButtonGroup, QStackedWidget, QSizePolicy, QToolButton, QFrame, QGroupBox
...
class PatientWidget(QWidget):
    def __init__(self, parent=None, import_folder_path: str = None, size_init_viewers=(1, 1),
        ...
        self.lst_series_name = set()  # use for check series. if read a series, we don't read again that.
        self.metadata_fixed = {}
        ...
        self.container_layout.setSpacing(0)  # Remove spacing between panels
        self.main_layout.addLayout(self.container_layout)
        self.main_layout.setContentsMargins(0, 0, 0, 0)

        # self.container_layout.addWidget(self.sidebar_left_panel())
        # self.container_layout.addWidget(self.thumbnail_layout_ui())

        self.sidebar = self.sidebar_layout_ui()
        self.container_layout.addWidget(self.sidebar)

        # # setup right panel layouts (left on vtk_widget)
        self.right_panel = QStackedWidget()
        self.right_panel.setFixedWidth(216)  # Calculated for 180px thumbnails + margins + scrollbar
        ...
        # Only start pipeline if we have a folder path
        if self.import_folder_path:
            self.pipeline_manager(caller=caller, size_init_viewers=size_init_viewers)
        # asyncio.create_task(self.pipeline_manager(caller=caller))
    ...
    def pipeline_manager(self, caller, size_init_viewers=(1, 1)):
        try:
            ...
            # No running event loop - run synchronously for now
            print("🔄 No event loop found, running pipeline synchronously...")
            
            if caller == CallerTypes.SERVER:
                # For server caller, just load thumbnails directly without async
    ...
    def load_thumbnails_sync(self):
        ...
        try:
            ...
            if self.import_folder_path:
                ...
                self.metadata_fixed['institution_name'] = 'N/A'  # Default value
                self.metadata_fixed['institution_name'] = 'N/A'  # Default value
        ...
        if option == "series":
            ...
            if not hasattr(self, 'ai_chat_widget'):
                try:
                    ...
                    self.right_panel.addWidget(self.ai_chat_widget)
                except Exception as e:
            ...
            try:
                self.right_panel.setCurrentWidget(self.ai_chat_widget)
            except Exception as e:
    ...
    def thumbnail_layout_ui(self):
        """Thumbnail panel with right panel styling"""
        # Main panel
        ...
        thumbnail_panel.setFixedWidth(216)  # Calculated for 180px thumbnails + margins + scrollbar
    ...
    def center_layout_ui(self):
        ...
        self.vtk_layout.setContentsMargins(0, 0, 0, 0)  # Remove margins to eliminate spacing
        self.vtk_layout.setSpacing(0)  # Remove spacing between grid items

        # first_node_viewer = self.new_viewer(default_thumb_index=0)
        # # self.set_viewer_to_main_viewer(first_node_viewer)
        # # self.change_container_border(0)
        # first_widget = first_node_viewer.widget
        # # self.lst_nodes_viewer.append(first_node_viewer)
        #
        # self.vtk_layout.addWidget(first_widget, 0, 0)
    ...
    def create_dummy_vtk_widget(self):
        ...
        vtk_dummy_widget = VTKWidget()
        # vtk_dummy_widget.render_window.Render()

        slider = QSlider(Qt.Vertical, vtk_dummy_widget)
        slider.setInvertedAppearance(True)
        slider.setStyleSheet("""
            QSlider {
                background: rgba(0, 0, 0, 1);
        ...
        layout.addWidget(vtk_dummy_widget, 0, 0)
        layout.addWidget(slider, 0, 0, alignment=Qt.AlignRight)

        container = QWidget()
        container.setLayout(layout)
        # Set default border for dummy viewport
        container.setStyleSheet("""
            QWidget {
                border: 2px solid #4a5568;
                border-radius: 5px;
    ...
    def change_container_border(self, id_vtk_widget):
        ...
        for node_viewer in self.lst_nodes_viewer:
            ...
            if node_viewer_selected.widget == node_viewer.widget:
                # Selected viewport - bright blue border with glow effect
                ...
                # Non-selected viewport - subtle gray border
    ...
    def create_new_vtk_widget(self, default_thumb_index):
        ...
        if flag_open_combine_viewer:
            vtk_widget.start_process_combine_series(
                vtk_widget_data, metadata, vtk_widget_data_2, metadata_2, default_thumb_index, id_new_vtk_widget,
                metadata_fixed=self.metadata_fixed)

        else:
            vtk_widget.start_process_series(
                vtk_image_data=vtk_widget_data, metadata=metadata, series_index=default_thumb_index,
    ...
    def change_series_on_viewer(self, series_index, flag_change_selected_widget=True,
        ...
        # Safety check for series_index
        if series_index < 0 or series_index >= len(self.lst_thumbnails_data):
            print(f"⚠️ Warning: series_index {series_index} out of range (0-{len(self.lst_thumbnails_data)-1})")
            return
        
        # Check if vtk_image_data exists
        if 'vtk_image_data' not in self.lst_thumbnails_data[series_index]:
            print(f"⚠️ vtk_image_data not available for series {series_index}, loading it now...")
            # Try to load the series data
            try:
                ...
                if study_uid:
                    series_path = check_series_study_exist(study_uid, str(series_index + 1))
                    if series_path and os.path.exists(series_path):
                        print(f"🔄 Loading series data from: {series_path}")
                        # Load the series data
                        from PacsClient.pacs.patient_tab.utils.image_io import load_images
                        
                        # load_images is a generator, so we need to get the first result
                        ...
                        self.lst_thumbnails_data[series_index]['vtk_image_data'] = vtk_image_data
                        self.lst_thumbnails_data[series_index]['metadata'] = metadata
                        print("✅ Series data loaded successfully")
                    else:
                        print(f"❌ Series path not found: {series_path}")
                        return
                else:
                    print("❌ No study_uid available")
                    return
            except Exception as load_error:
                print(f"❌ Error loading series data: {load_error}")
                import traceback
                traceback.print_exc()
                return
        ...
        # check for series has depended on series
        for i in range(len(self.lst_thumbnails_data)):
            if i == series_index:
                continue
            series_name_2 = self.lst_thumbnails_data[i]['metadata']['series']['series_name']
        ...
        if flag_change_selected_widget:  # change on first viewer
            # Safety check for selected_widget
            if self.selected_widget is None:
                print("⚠️ Warning: selected_widget is None, cannot switch series")
                return
                
            # selected_widget is already a VTKWidget, find its NodeViewer
            selected_node = None
            ...
            if selected_node is None:
                print("⚠️ Warning: Could not find NodeViewer for selected_widget")
                return
                
            flag_switch = self.selected_widget.switch_series(vtk_image_data, metadata, series_index, vtk_widget_data_2,
                                                           metadata_2, self.metadata_fixed)
            vtk_widget = self.selected_widget  # Use the selected VTKWidget
            slider = selected_node.slider  # Get slider from NodeViewer

        else:  # change on selected viewer
            # vtk_widget should already be a VTKWidget, but let's be safe
            if hasattr(vtk_widget, 'vtk_widget'):
                # If it's a NodeViewer, get the actual VTKWidget and slider
                actual_vtk_widget = vtk_widget.vtk_widget
                flag_switch = actual_vtk_widget.switch_series(vtk_image_data, metadata, series_index, vtk_widget_data_2,
                                                             metadata_2, self.metadata_fixed)
                slider = vtk_widget.slider  # Get slider from NodeViewer
                vtk_widget = actual_vtk_widget
            else:
                # It's already a VTKWidget, slider should be provided
                flag_switch = vtk_widget.switch_series(vtk_image_data, metadata, series_index, vtk_widget_data_2,
                                                       metadata_2, self.metadata_fixed)
                # slider should already be provided as parameter

        if flag_switch is True:
            self.reset_slider(vtk_widget, slider)
            self.toolbar_manager.turn_off_all_tools()
            
            # Apply reset like toolbar reset after series change with delay
            from PySide6.QtCore import QTimer
            
            if flag_change_selected_widget and self.selected_widget:
                # Find the node viewer for the selected widget
                for node in self.lst_nodes_viewer:
                    if node.vtk_widget == self.selected_widget:
                        QTimer.singleShot(100, lambda n=node: self._apply_reset_to_viewer(n))
                        break
            elif not flag_change_selected_widget and vtk_widget:
                # Find the node viewer for the specific widget
                for node in self.lst_nodes_viewer:
                    if node.vtk_widget == vtk_widget:
                        QTimer.singleShot(100, lambda n=node: self._apply_reset_to_viewer(n))
                        break
            
            # Update spinner position and corners after series change
            if self.selected_widget:
    ...
    def reset_slider(self, vtk_widget: VTKWidget, slider: QSlider):
        # set slicer again. because interactor_style changed and we need set slicer in interactor_style
        vtk_widget.set_slider(slider)
        count_slices = vtk_widget.get_count_of_slices()
    ...
    async def start_auto_thumbnail_download(self):
        ...
        try:
            ...
            if not study_uid:
                print("❌ No study_uid available for auto-download")
