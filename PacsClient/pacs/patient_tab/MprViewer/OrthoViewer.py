...
class OrthoViewer(VtkViewer):
    ...
    def __init__(self, vtkBaseClass:VtkBase, orientation, label:str="Orthogonal Viewer"):
        ...
        # Set interpolation on the reslice representation
        # Use Linear for good quality (default)
        try:
            reslice = self.resliceCursorRep.GetReslice()
            if reslice:
    ...
    def update(self, reset_camera=False):
        super().update(reset_camera=reset_camera)
    
    # Render
    def render(self, reset_camera=False):
        super().render(reset_camera=reset_camera)

    # Events
    def add_observers(self):
        # Update when reslice axes change (end of interaction)
        self.resliceCursorWidget.AddObserver(vtk.vtkResliceCursorWidget.ResliceAxesChangedEvent, self.commandSliceSelect)
        # Also update during interaction for real-time feedback
        self.resliceCursorWidget.AddObserver(vtk.vtkCommand.InteractionEvent, self.commandSliceSelect)
