...
**Current Stable Version:** v3.5.5 (2026-07-25) - Minor release: multi-frame DICOM geometry (OPT-42 — cine/enhanced MR-CT), Offline Service patient-delete management, Report Editor "Previous Exams" (cross-PatientID) header, previous-exam mid-download grow (OPT-39), prewarm idle-gate hardening (OPT-41), manual-download study-discovery fix (OPT-40), a −670-line download-client cleanup, and the first test-suite KPI baseline. See `docs/releases/VERSION_3.5.5_RELEASE.md`.

**Previous Stable:** v3.5.4 (2026-07-19) - Minor release: automatic incremental update system (OPT-38), Agent Gateway (mobile/MCP connectivity), demographic DICOM-tag editor, viewport overlay reads the displayed image's own DICOM identity (clinical safety), sortable Status/Report columns, "Imported On" column. See `docs/releases/VERSION_3.5.4_RELEASE.md`.
...
**Previous Stable:** v3.3.1 (2026-06-16) - Minor release: latest stable consolidation build with current local changes. See `docs/releases/VERSION_3.3.1_RELEASE.md`.

**Previous Stable:** v3.3.0 (2026-06-15) - Minor release: latest stable consolidation build with current local changes. See `docs/releases/VERSION_3.3.0_RELEASE.md`.

**Previous Stable:** v3.2.9 (2026-06-14) - Minor release: latest stable consolidation build with current local changes. See `docs/releases/VERSION_3.2.9_RELEASE.md`.

**Previous Stable:** v3.2.8 (2026-06-13) - Minor release: latest stable consolidation build with current local changes. See `docs/releases/VERSION_3.2.8_RELEASE.md`.

**Previous Stable:** v3.2.7 (2026-06-11) - Minor release: latest stable consolidation build with current local changes. See `docs/releases/VERSION_3.2.7_RELEASE.md`.

**Previous Stable:** v3.2.6 (2026-06-09) - Minor release: latest stable consolidation build with current local changes. See `docs/releases/VERSION_3.2.6_RELEASE.md`.

**Previous Stable:** v3.2.5 (2026-06-08) - Minor release: latest stable consolidation build with local updates. See `docs/releases/VERSION_3.2.5_RELEASE.md`.

**Previous Stable:** v3.2.3 (2026-06-07) - Minor release: v3.2.3 production updates. See `docs/releases/VERSION_3.2.3_RELEASE.md`.

**Previous Stable:** v3.2.0 (2026-06-02) - Major release: v3.2.0 production milestone. See `docs/releases/VERSION_3.2.0_RELEASE.md`.

**Previous Stable:** v3.1.9 (2026-06-02) - Minor release: v3.1.9 production updates. See `docs/releases/VERSION_3.1.9_RELEASE.md`.
...
- **R17 � FAST pipeline MUST force `preserve_dimensions=True` when invoking the PooyanPacs OpenCV filter, AND `_numpy_to_qimage_gray/rgb` MUST derive width/height/bytesPerLine from `arr.shape` � not from caller-supplied dims** (v2.3.8). Root cause of "two brains overlapping with scan-lines on stationary viewer, correct on stacking viewer" artifact: `pooyan_filter_center` with `preserve_dimensions=False` + `small_threshold=280` enlarges images narrower than 280 px by 2� on both axes (e.g. 256�184 ? 512�368) � this is documented C# PooyanPacs behavior for a display backbuffer that does not exist in the FAST Qt path. `Lightweight2DPipeline._render_frame_uncached` then fed this enlarged buffer to `_numpy_to_qimage_gray(disp, sm.cols, sm.rows)` using the ORIGINAL `sm.cols=184, sm.rows=256`. QImage interpreted 184 as both image width and `bytesPerLine`, reading a 368-byte-wide array as if each row were 184 bytes ? exactly the wrapped/doubled/scan-line pattern. Why stacking appeared normal: `filter_enabled = opencv_filter_enabled AND (NOT fast_interaction OR interaction_type=='wheel')` � during stack-drag `filter_enabled=False`, so the filter was skipped and dimensions remained (256, 184). On release, the settled re-render applied the filter and produced the corrupted output. **Canonical fix**: (1) force `preserve_dimensions=True` at the FAST call site in `modules/viewer/fast/lightweight_2d_pipeline.py::_render_frame_uncached` � the 2� enlargement is redundant in the FAST path because `QGraphicsView` handles display scaling natively; (2) make `_numpy_to_qimage_gray` derive `actual_w, actual_h = arr.shape[:2]` and use those for BOTH image dimensions AND `bytesPerLine`, logging an error if the caller's supplied dims disagree (defense-in-depth for any future dimension-changing filter); (3) set `"preserve_dimensions": true` in `config/pooyan_opencv_filter.json` so config-sourced default agrees. The plugin-package copy at `builder/plugin package/packages/viewer/payload/python/modules/viewer/fast/lightweight_2d_pipeline.py` MUST be kept in sync or production builds regress. Advanced (VTK) path is unaffected � it handles filter-induced dimension changes through `_preprocess_vtk_image_data`'s `vtkImageResample`. Do NOT add new callers that pass hard-coded `sm.cols/sm.rows` to the QImage helpers; always pass `disp.shape[1]/disp.shape[0]` if you must pass anything. Do NOT relax `preserve_dimensions` back to `False` at the FAST call site. Diagnostic signature of regression: image looks correct while stacking/wheel-scrolling but becomes "wrapped / two-brains / horizontal scan-lines" on release for any series with width OR height < 280 px (common for MR).\n- **R18 � Prefetch pre-queue cancellation gates AND direction-flip target invalidation** (v2.4.7 / plan F3.1+F3.2). `Lightweight2DPipeline._submit_prefetch(idx, generation, request_epoch=0)` rejects stale tasks BEFORE `executor.submit()` (saves IPC + pickle + worker-dispatch cost) using three gates evaluated under `_prefetch_lock`: (1) **generation gate** � `generation > 0 and generation != self._prefetch_generation` rejects (W/L change or series close already bumped generation); (2) **request-epoch gate** � `request_epoch > 0 and request_epoch != self._prefetch_request_epoch and idx not in self._active_prefetch_targets` rejects (neighborhood replaced; only admit if idx is still in the latest active target set, which preserves overlap-prefetch when scroll continues in the same direction); (3) **distance gate** � `abs(idx - self._current_index) > _max_distance` rejects, where `_max_distance = 6` during `_fast_interaction` and `_config.prefetch_radius` otherwise. Each rejection increments `PerfMetrics.cancelled_task`. F3.2 layers on top: `_prefetch_around` tracks `self._last_prefetch_direction` (set on every `direction != 0` call) and forces `_prefetch_request_epoch += 1 + _active_prefetch_targets = uncached_targets` when the new direction differs from the last non-zero one � even if the new neighborhood overlaps the old. This guarantees that when the user reverses scroll direction mid-drag, every queued task in the OLD direction fails gate (2) and gets cancelled pre-queue. `_last_prefetch_direction` is reset to 0 in `close_series`, `begin_protected_drag_session`, and the W/L-change path (mirroring `_last_prefetch_center` resets). Post-decode guards in `_decode_into_cache` remain intact as a safety net but should be reached only by tasks that started before the gate was evaluable. Do NOT remove the pre-queue gates � they are the difference between cancelled-task-ratio ?10% (post-decode only) and ?50%+ (pre-queue + post-decode). Do NOT update `_last_prefetch_direction` on `direction == 0` calls (idle/centering prefetch) � that would silently overwrite the tracked direction and make legitimate same-direction continuation look like a flip. The plugin-package copy at `builder/plugin package/packages/viewer/payload/python/modules/viewer/fast/lightweight_2d_pipeline.py` MUST be kept in sync. Tests: `tests/viewer/test_prefetch_pre_queue_cancel.py` (9, F3.1) + `tests/viewer/test_b34_interaction_aware_policy.py::TestDirectionReversal` (4, F3.2).\n- **R15 � Advanced (VTK) viewer participates in the unified protected-interaction latch** (v2.3.8). `modules/viewer/fast/ui_throttle.py` now exposes `record_advanced_protected_interaction(active, *, grace_ms, source)` writing a parallel `_ADVANCED_PROTECTED_*` state. `is_protected_drag_active()` returns `True` if EITHER the FAST latch (`_PROTECTED_DRAG_*`) OR the Advanced latch (`_ADVANCED_PROTECTED_*`) is active/within its grace window. The Advanced viewer calls this from exactly two sites in `vtk_widget/_vw_scroll.py`: (1) `_flush_pending_wheel_slice_impl` after the `_in_wheel_scroll`/`_in_stack_scroll` flags are set ? `(True, grace_ms=2500.0)` acting as begin + per-frame keepalive (2500 ms covers the 2000 ms `_gc_reenable_timer` + margin), and (2) `_reenable_gc_impl` at the end-of-burst path ? `(False, grace_ms=250.0)` with a tail grace mirroring FAST's `record_protected_drag`. The end-of-burst call runs **outside** the `self._gc_suppressed` guard because stack-drag bursts start the GC re-enable timer without flipping `_gc_suppressed`. Because R3 (CACHE_WARM denial, PREFETCH denial unless priority?1), R4 (progressive grow defer � architecturally no-op in Advanced; docstring states Advanced progressive is unreachable in production), and R5 (DM `_apply_throttled_progress` skip) all gate on `is_protected_drag_active()`, the read sites require zero changes � extension is automatic. The latches are kept **separate** (not unified into one flag) so `[PROTECTED_DRAG]` vs `[PROTECTED_ADVANCED]` log lines remain distinguishable for diagnostics. R13 cross-process `.drag_active` flag is touched on every Advanced `active=True` call, so when `AIPACS_DRAG_SUBPROC_THROTTLE=1` is enabled, Advanced drags throttle the download subprocess identically to FAST drags. Do NOT add a second Advanced release call in the wheel-coalesce `finally:` block � the `_gc_reenable_timer` is the single source of truth for "burst tail", and adding a synchronous release there would defeat the 250 ms tail grace. Do NOT consolidate the FAST and Advanced latches into one variable � separate state is required for the `[PROTECTED_*]` log disambiguation and for independent `reset_ui_tick_baseline()` semantics. Verify with `tests/viewer/test_advanced_protected_interaction.py` (7 tests).\n- **R19 � DM priority-handoff `[INTENT_PRIORITY]` instrumentation is observation-only; verbose tags are env-gated; emits at WARNING level** (v2.4.7 / plan F3.5.1; **level corrected 2026-04-30** after live log pid=32956 produced 0 INTENT_PRIORITY lines despite a clear `Priority start retry exhausted` event). `SeriesIntentCoordinator._emit_intent_priority(tag, ...)` produces a stable structured log at six points in the priority retry chain (`begin`, `tick`, `defer`, `recover`, `exhaust`, `started`). The line format is `[INTENT_PRIORITY] tag=<TAG> study=<UID> series=<SN> attempt=<N>/<M> recovery=<BOOL> pool_busy=<BOOL> pool_capacity=<U>/<T> state=<S> auto_paused=<BOOL> elapsed_ms=<INT> token=<INT> [branch=<B>]` emitted via **`logger.warning(...)` with `extra={"component": "download"}`** � WARNING (not INFO) is mandatory because `_DEFAULT_COMPONENT_THRESHOLDS["download"] = logging.WARNING` in `PacsClient/utils/diagnostic_logging.py`; INFO emits with `component=download` are silently dropped before reaching `download_diagnostics.log` (same trap as R13 `[SP]` and R22 `[DM_REBUILD]`). `tick` and `defer` are verbose-only and emitted ONLY when `AIPACS_INTENT_PRIORITY_TRACE=1` (default `0` keeps prod logs quiet); `begin`, `recover`, `exhaust`, `started` always emit. `recover branch=primary` and `exhaust branch=primary` both increment `primary_exhaust_count` in the KPI parser (primary-chain expiration is the user-visible primary-exhaust event regardless of whether recovery follows); `exhaust branch=recovery` (or missing branch) increments `recovery_exhaust_count`. `_priority_retry_started_ms[study_uid]` is set in `_begin_priority_retry` (alongside `_priority_retry_tokens[study_uid]`) and cleared in `_clear_priority_retry`; on success path `_clear_priority_retry` runs after `start_download_worker` returns True; on exhaust path it runs at the end of recovery before classifying as auto-pause/preemption-expected. Pool capacity is read via `len(getattr(self.worker_pool, "active_workers", {}) or {})` and `getattr(self.worker_pool, "max_workers", 0)` � never index `active_workers` directly. The plugin-package copy at `builder/plugin package/packages/download_manager/payload/python/modules/download_manager/coordinator/series_intent_coordinator.py` MUST stay SHA-equal to the canonical file. The KPI parser in `tools/performance/clearcanvas_aipacs_kpi_harness.py::parse_priority_handoff_log_text` (CLI: `parse-priority-handoff-log`) is contract-tested in `tests/performance/test_priority_handoff_kpi_parser.py` (15 tests) � any change to the emit format MUST be mirrored there or the F3.5.x baseline pipeline breaks. Instrumentation behavior (begin ? started monotonic elapsed_ms, exhaust clears `_priority_retry_started_ms`, dedup, trace gate) is verified in `tests/download_manager/test_priority_handoff_instrumentation.py` (7 tests). Do NOT add new emit sites without also extending the parser regex `_INTENT_PRIORITY_RE` and at least one round-trip test. Do NOT remove the `branch=` field � it is the only signal that distinguishes primary-chain exit from recovery-chain exit and is required for triaging the "Priority start retry exhausted" warnings observed in 3 production sessions on 2026-04-28.\n- **R20 � V2 wall-clock priority-handoff retry ships default-off behind `AIPACS_INTENT_HANDOFF_V2`** (v2.4.7 / plan F3.5.2). When the env var is unset or not `"1"`, `SeriesIntentCoordinator.schedule_priority_start_retry` runs the legacy 90�200 ms primary chain + 5 s gap + 3�3000 ms recovery chain unchanged (R19 contract). When `=1` AND the call is a chain-start (`_token is None`), the coordinator runs `_schedule_priority_start_retry_v2` instead: a single 250 ms-cadence loop with a 60 s wall-clock budget (`INTENT_HANDOFF_HARD_TIMEOUT_MS`, `INTENT_HANDOFF_V2_INTERVAL_MS`). V2 emits the existing `[INTENT_PRIORITY]` lines tagged `branch=v2` plus an optional `reason=<R>` field on `defer reason=reclaimed` and on every `exhaust` path (one of `pool_busy|reclaimed|state_lost|timeout`). **Reclamation-race handling**: when `WorkerPool.can_add_worker()` returns True but `_start_download_worker()` returns False, V2 emits `tag=defer pool_busy=False branch=v2 reason=reclaimed` and ALSO calls `_defer(0, _start_next_pending)` to nudge the central scheduler � the legacy path silently treats this as a normal retry. **CAS state promotion**: PAUSED?PENDING flips through the new additive `DownloadStateStore.update_if_status(study_uid, expected_status, new_status, **extra_changes)` helper, eliminating the read-modify-write race against concurrent state writers (the helper returns False on missing state or mismatched expected). The V2 fork is decided at chain-start only (legacy in-flight chains are not yanked mid-flight if env flips); env is read every chain start (not at module import), so tests can monkeypatch via `monkeypatch.setenv` without re-import. KPI parser (`tools/performance/clearcanvas_aipacs_kpi_harness.py::parse_priority_handoff_log_text`) gained `v2_begin_count`, `v2_started_count`, `v2_defer_reclaimed_count`, `v2_exhaust_pool_busy_count`, `v2_exhaust_reclaimed_count`, `v2_exhaust_state_lost_count`, `v2_exhaust_timeout_count`, and `overlap_priority_handoff_v2_total_exhaust_count`. All zero on the legacy path. Synthetic baselines: `tools/performance/synthetic_priority_handoff_runner.py --mode both` produces `generated-files/benchmarks/priority_handoff_v2_pre.json` (env=0) and `priority_handoff_v2_post.json` (env=1). Pre baseline shows 20 `primary_exhaust` events on a 25 s peer-held-slot scenario (legacy 18 s primary cap is hit before the peer release); post baseline shows 0 primary/recovery exhausts and 12 `v2_defer_reclaimed_count` (3 race-flagged handoffs � 4 ticks each in the 1 s reclamation window). The plugin-package copy at `builder/plugin package/packages/download_manager/payload/python/modules/download_manager/...` MUST stay SHA-equal to the canonical file. Tests: `tests/download_manager/test_priority_handoff_v2.py` (9 tests, V2 path); `tests/performance/test_priority_handoff_kpi_parser.py` (25 tests, including 10 V2 round-trips). Do NOT enable V2 by default without first running the full DM stress (H1�H10) + load (L1�L11) + synthetic-runner pre/post comparison on at least 2 PCs (PC A + PC B per cross-PC workflow); the F3.5.2 ship is observation-only on PC A. Do NOT remove the `_token is None` gate on the V2 fork � without it a CRITICAL re-entry on an in-flight legacy chain would yank the chain mid-flight and double-emit `begin`. Do NOT collapse the four exhaust reasons (`pool_busy|reclaimed|state_lost|timeout`) into a single counter � they classify distinct production failure modes and are partitioned in the KPI harness.\n- **R21 � `[SLOT_TIMING]` G6 instrumentation is observation-only with a stable emit format** (v2.4.7 / plan G6 � silent main-thread blocker triage). `modules/viewer/fast/slot_timing.py` exposes three primitives: `emit_slot_timing(tag, duration_ms, *, drag_active=None, series=None, extra=None, force=False) -> bool`, the `@slot_timing(tag, *, series_arg=None, extra_factory=None)` decorator, and the `time_slot(tag, *, series=None, extra=None, force=False)` context manager. The decorator and context manager re-check `is_slot_timing_enabled()` at call time (NOT at decoration time) so monkeypatched env in tests behaves correctly. Emit format is locked: `[SLOT_TIMING] tag=<TAG> duration_ms=<F.3> drag_active=<True|False> threshold_ms=<F.1> series=<SN|none> extra=<k1=v1;k2=v2>` with `extra={"component": "viewer"}` so the queued listener admits it at INFO threshold. Default thresholds: idle 30 ms (`AIPACS_SLOT_TIMING_THRESHOLD_MS`), drag 8 ms (`AIPACS_SLOT_TIMING_DRAG_THRESHOLD_MS`); kill switch `AIPACS_SLOT_TIMING_TRACE=0`. Field separators in `extra` values are sanitized: `;` ? `,` and `=` ? `:` so format never breaks. Drag-active resolution is via lazy import of `ui_throttle.is_protected_drag_active()`, with try/except ? False fallback (decorator must NOT fail when the latch module is unavailable, e.g. headless tests). The decorator MUST swallow `extra_factory` exceptions, MUST emit even when the wrapped function raises, and MUST NOT swallow the wrapped function's exception (re-raise after emit). Currently wired callsites: `PacsClient/pacs/patient_tab/utils/thumbnail_manager.py` (4 methods: `apply_border_states_new`, `update_series_progress`, `start_series_download`, `complete_series_download`); `PacsClient/pacs/workstation_ui/home_ui/home_download_service.py::on_series_completed` (refactored to `_on_series_completed_impl` + `time_slot` wrap); `modules/zeta_boost/cache_engine/_zb_lifecycle.py::notify_global_download_stop` (inline `time_slot`); `PacsClient/pacs/patient_tab/ui/patient_ui/_vc_progressive.py::_finalize_progressive_series` (module-level helper at line ~548, NOT the mixin one � module-level path is what tests bind on lightweight controllers; emit fires before `return True` with extra `source/force/viewers/final_count`). KPI parser at `tools/performance/clearcanvas_aipacs_kpi_harness.py::parse_slot_timing_log_text` returns per-tag drag-only and overall percentiles + `top_drag_tags` (top 5 by `drag_total_ms`, only nonzero) + scalar fields `overlap_slot_timing_drag_blocked_ms_total`, `overlap_slot_timing_worst_drag_call_ms`, `overlap_slot_timing_worst_drag_tag`. Tests: `tests/viewer/test_slot_timing.py` (26 � decorator + CM + env + sanitization + exception safety) and `tests/performance/test_slot_timing_kpi_parser.py` (15 � round-trip via real helper, prefix tolerance, malformed lines silently dropped, ranking, p50/p95/max). The plugin-package copy at `builder/plugin package/packages/viewer/payload/python/modules/viewer/fast/slot_timing.py` MUST stay SHA-equal to the canonical file (and the zeta_boost mirror at `builder/plugin package/packages/zeta_boost/payload/python/modules/zeta_boost/cache_engine/_zb_lifecycle.py` likewise for the wired emit). thumbnail_manager.py / home_download_service.py / _vc_progressive.py have NO plugin-package mirrors � verified by `Get-ChildItem` 2026-04-29 � so only the helper + zeta_boost copy need parity. Do NOT add new emit sites without also extending `_SLOT_TIMING_RE` if you change format, or adding at least one round-trip test. Do NOT make this rule defer or skip work � G6 is purely observational; defers based on G6 data ship as G7+ rules with explicit defer instrumentation following the F9/F10 pattern. Do NOT cache the `is_slot_timing_enabled()` check at decoration time � tests rely on the call-time recheck.\n- **R22 � DM table-rebuild storm fix: `blockSignals` discipline + reentrancy guard + `[DM_REBUILD]` instrumentation** (v2.4.7 / plan `docs/plans/performance/DM_TABLE_REBUILD_STORM_2026-04-29.md`). Root cause: `_DmDetailsMixin._clear_details_panel` in `modules/download_manager/ui/widget/_dm_details.py` writes `self.priority_combo.setCurrentText("Normal")` to reset the inspector. When this runs inside `_refresh_table_order` (called from `SeriesIntentCoordinator.request_critical_series` on every drag-drop), Qt fires `currentTextChanged` synchronously, invoking `_DmControlsMixin._on_priority_changed` which calls `state_store.update(study_uid, priority=NORMAL)` � silently demoting the freshly-promoted CRITICAL study back to NORMAL AND triggering a recursive `_refresh_table_order` rebuild. Live evidence: 1264 main-thread stalls ?50ms (91 ?1s, 14 ?5s) in one drag-heavy session; F11 stack capture pinpointed `_dm_details.py:_refresh_table_order` as the culprit frame. **Canonical fix (G8.1)**: wrap the combo write in `try: priority_combo.blockSignals(True); priority_combo.setCurrentText("Normal"); finally: priority_combo.blockSignals(False)` � same pattern as the established precedent in `_dm_controls.py:374-376` (rejection path) and `_dm_details.py:374-376`. **Defense-in-depth (G8.2)**: `_refresh_table_order` carries a `self._refresh_table_order_in_progress` boolean reentrancy guard; recursive entry returns early after emitting `[DM_REBUILD] event=reenter_skip depth=N caller=<file:func>`. **Observability (G7)**: every entry/exit emits structured logs `[DM_REBUILD] event=enter depth=<N> caller=<file:func>` + `[DM_REBUILD] event=exit depth=<N> duration_ms=<F.3> rows=<R> caller=<file:func>` at **WARNING** level with `extra={"component": "download"}`. WARNING (not INFO) is mandatory because `_DEFAULT_COMPONENT_THRESHOLDS["download"] = logging.WARNING` in `PacsClient/utils/diagnostic_logging.py` � INFO-level emits with `component=download` are silently dropped before reaching `download_diagnostics.log` (this is the same trap documented in R13 for subprocess `[SP]` lines, and is why the first live capture of R22 produced zero hits despite drag-drop activity). The optional `_dm_rebuild_caller_frame()` static helper uses `inspect.stack()[2]` to identify the caller (one cheap call per rebuild). Companion tag `[DM_PRIORITY_TRANSITION] event=combo_changed new=<P> study=<UID> during_rebuild=<True|False>` is emitted at the top of `_on_priority_changed` also at WARNING level for the same reason; **`during_rebuild=True` is the regression alarm** (post-fix it must always be False � only user clicks survive). KPI parser `tools/performance/clearcanvas_aipacs_kpi_harness.py::parse_dm_rebuild_log_text` returns `dm_rebuild_count`, `dm_rebuild_recursive_count` (depth?2 exits), `dm_rebuild_reenter_skip_count`, `dm_rebuild_max_depth`, `dm_rebuild_duration_p50/p95/max_ms`, `dm_rebuild_per_session_total_ms`, `top_callers` (top 5 by total_ms). Companion `parse_dm_priority_transition_log_text` exposes `priority_combo_signal_during_rebuild_count`. KPI targets: `dm_rebuild_recursive_count=0`, `dm_rebuild_reenter_skip_count=0`, `dm_rebuild_duration_p95_ms<80`, `dm_rebuild_duration_max_ms<200`, `priority_combo_signal_during_rebuild_count=0`; `MAIN_THREAD_STALL` count ?200ms must drop substantially (live target: 213?<30 in a comparable drag-heavy session). The plugin-package copies at `builder/plugin package/packages/download_manager/payload/python/modules/download_manager/ui/widget/_dm_details.py` and `_dm_controls.py` MUST stay SHA-equal to canonical (verified via `Get-FileHash`). Tests: `tests/download_manager/test_dm_rebuild_recursion.py` (6 � source contracts + live `QComboBox` signal proof + end-to-end coordinator priority-corruption regression with `_ghost_signal_present_in_production()` probe + reentrancy guard contract); `tests/performance/test_dm_rebuild_kpi_parser.py` (14 � round-trip + ranking + percentiles + production emit format + `diagnostic_logging` prefix tolerance). Do NOT remove the `blockSignals` wrap on line 233 � it is the load-bearing fix; the reentrancy guard is the safety net, not the primary mechanism. Do NOT remove the `during_rebuild=` field from `[DM_PRIORITY_TRANSITION]` � it is the only signal that distinguishes a regression of the ghost-signal bug from legitimate user clicks. Do NOT log per-row "Adding download row" / "Restoring selection" at INFO inside `_refresh_table_order` � that was the v2.4.6 spam pattern, demoted to DEBUG in this rule. User's offered Plan B ("remove CRITICAL priority entirely; use HIGH on patient open + NORMAL when not opened") was considered and **deferred** because the surgical fix eliminates both the recursion AND the priority demotion without losing the CRITICAL semantics required by drag-drop preemption. Plan B remains documented as a fallback in the plan doc.\n- **R23 � Typed structured-logging emit helpers + lint gate (silent-drop bug class)** (2026-04-30 / plan `docs/plans/ARCHITECTURE_REVIEW_2026-04-30.md` Phase 0). Root cause shared by R13 (`[SP]`), R19 (`[INTENT_PRIORITY]`), R22 (`[DM_REBUILD]`): `_DEFAULT_COMPONENT_THRESHOLDS["download"] = logging.WARNING` in `PacsClient/utils/diagnostic_logging.py` silently drops any `logger.info(..., extra={"component": "download"})` emit before it reaches `download_diagnostics.log`. Each of the three bugs was discovered only from a live log producing zero hits despite clear evidence of the underlying event. **Canonical fix**: `PacsClient/utils/structured_logging.py` exports typed helpers `emit_download_event(logger, tag, *, level=None, fields=None, extra=None)` (defaults WARNING), `emit_ipc_event` / `emit_viewer_event` / `emit_zetaboost_event` / `emit_db_event` / `emit_ui_event` (default INFO). Every helper sets `extra["component"]` internally AND promotes the level to the component minimum from `MIN_LEVEL_BY_COMPONENT` (mirror of the diagnostic_logging table � synced by `test_min_level_table_matches_diagnostic_logging`). Body format is `[TAG] k=v k=v` with field sanitization (`;`?`,`, `=`?`:`, `\\n`/`\\r`?space � matches R21 `slot_timing.py` contract). Float default precision is 3 decimals. None values dropped from body; bool serialized as `True`/`False` literal. **Lint gate**: `tests/utils/test_structured_logging_lint.py::test_no_silent_drop_violations` scans `PacsClient/`, `modules/`, `database/` for any `logger.info(..., extra={"component": "<comp>"})` (or `.warning`/`.error`/`.critical`/`.debug`) and fails when the level is `>= INFO` AND below `MIN_LEVEL_BY_COMPONENT[comp]`. Excluded paths: `builder/plugin package/`, `tests/`, `__pycache__`, `.venv*`. Excluded files: `PacsClient/utils/structured_logging.py` itself (its docstring contains an example of the bad pattern). Per-line opt-out: append `# noqa: structured-logging` to the offending source line. DEBUG is always permitted (developers explicitly want DEBUG invisible); only INFO+ falling below the component minimum is flagged. Scanner uses paren-balanced `_find_call_end` so it handles multi-line `logger.info(\\n  ..., \\n  extra=...)` correctly. Total scan time ~36 s for the full repo. **First-pass cleanup (2026-04-30)**: 20 production violations were fixed � 16 in `modules/download_manager/network/socket_client.py`, 2 in `modules/download_manager/ui/widget/_dm_workers.py`, 1 in `modules/download_manager/workers/download_process_worker.py`, 1 in `PacsClient/utils/diagnostic_logging.py`. Fix policy: KPI/summary/retry events bumped INFO ? WARNING (preserves visibility intent); routine chatter (`SocketDicomClient initialized`, `?? Receiving response body`, `?? Starting batch`, `?? Batch N: Got M instances`) demoted INFO ? DEBUG (silent on purpose, intent now explicit). All 3 `download_manager`-tier files mirrored to `builder/plugin package/packages/download_manager/payload/python/modules/download_manager/...`; `PacsClient/utils/diagnostic_logging.py` is PacsClient-tier and has no plugin-package mirror. Do NOT add new `logger.<level>(..., extra={"component": ...})` callsites without either (a) using a typed helper, (b) ensuring the level meets the component minimum, or (c) adding `# noqa: structured-logging` with a comment explaining why. Do NOT modify `MIN_LEVEL_BY_COMPONENT` without also updating `_DEFAULT_COMPONENT_THRESHOLDS` � `test_min_level_table_matches_diagnostic_logging` enforces parity. Do NOT remove the lint test from CI/dev workflow � it is the only mechanism preventing R13/R19/R22-class regressions. Tests: `tests/utils/test_structured_logging.py` (17 � helper unit tests including level promotion, sanitization, body format, sync check) and `tests/utils/test_structured_logging_lint.py` (6 � production scan + 5 scanner self-tests).\n\n
- **R24 � Plugin payload namespace-shadow invariant + build-time enforcement** (2026-05-02 / structural hardening). Root cause of the recurring `ModuleNotFoundError: No module named 'modules.mpr.curved_mpr'` crash pattern: any plugin that ships a `modules/<X>/__init__.py` where `<X>` also exists in the engine bundle but the plugin copy is INCOMPLETE (missing engine subpackages) makes `modules.X` a regular package pointing only at the plugin's partial tree. When activated via `activate_optional_module_runtime()` in `aipacs_runtime.py`, the plugin path is added to `modules.__path__`; if it is prepended, the engine's complete `modules.X` package is shadowed and all imports of missing subpackages raise `ModuleNotFoundError`. **Three-layer hardening (2026-05-02):** (1) `builder/plugin_package_registry.py::_normalize_definition()` raises `ValueError` if any `source_paths` item points directly at an `__init__.py` file; (2) `builder/materialize_plugin_packages.py::_validate_plugin_no_namespace_shadow()` and `builder/build_release.py::_validate_staged_plugin_no_namespace_shadow()` both walk the `payload/python/modules/` output tree and raise `ValueError` with `[SHADOW_CHECK]` tag when `modules/__init__.py` exists at the top level, or `modules/<X>/__init__.py` exists AND the engine's `modules/<X>/` has Python subpackage directories not present in the plugin copy (real subpackages only � `__pycache__` and dot-dirs excluded); (3) `aipacs_runtime.py::activate_optional_module_runtime()` appends (never prepends) plugin paths to `sys.path` and `modules.__path__`, and emits `[R24_SANITY] WARNING` if a plugin path ever ends up at index 0 of `modules.__path__`. **Plugin payload invariant**: `payload/python/modules/` is a namespace package (no `__init__.py`); plugin-contributed `modules/<X>/` entries are ADDITIVE � they introduce brand-new sub-modules not in the engine and must never provide a partial replacement. Do NOT add `__init__.py` to any plugin's `source_paths`. Do NOT change `sys.path.append` back to `sys.path.insert(0, ...)` in `activate_optional_module_runtime`. Do NOT remove `_validate_*_no_namespace_shadow` calls from copy/staging code paths. Do NOT relax the `__pycache__`-exclusion logic in the shadow checks.
- **R25 � Backend selection persistence + precedence contract** (2026-05-03 / v2.4.8c hotfix). The installed app must preserve user-selected viewer backend across restarts and must honor explicit global Advanced mode in Settings. Two load-bearing constraints: (1) `aipacs_runtime.py::seed_user_config_defaults()` may seed missing files but MUST NOT overwrite existing `%APPDATA%\AIPacs\config\viewer_backend_settings.json`; (2) `PacsClient/pacs/patient_tab/ui/patient_ui/_vc_backend.py::_get_requested_viewer_backend()` may apply `parent_widget.viewer_backend_override` only when configured global backend is FAST (`BACKEND_PYDICOM_QT` / legacy `BACKEND_PYDICOM`), and MUST NOT let that override supersede configured Advanced (`BACKEND_VTK`). Do NOT revert to unconditional copy in seeding. Do NOT restore unconditional parent-override precedence. Regression signal: user selects Advanced in Settings but viewer still resolves to FAST after reopen/restart. Guard tests live in `tests/viewer/test_fast_viewer_pipeline.py` (configured Advanced beats override; override applies in FAST only).
- **R26 � FAST stack interaction keeps OpenCV filter enabled (wheel + drag)** (2026-05-07 / Phase 3 closure hardening). `Lightweight2DPipeline.get_rendered_frame()` must keep `filter_enabled = bool(self._config.opencv_filter_enabled)` for stack interaction modes; do NOT reintroduce drag-time filter suppression logic (e.g. `and (not self._fast_interaction or interaction_type=='wheel')`). Drag-mode pixel-quality contract now validates against per-case golden hashes (`tests/viewer/test_overlap_pixel_quality_drag.py`) so `filter_on_*` drag runs must match filter-on references while still satisfying surrogate validity/proximity and settle exactness. Any FAST filter-policy change requires rerunning `tools/dev/run_overlap_regression.ps1` and mirroring plugin payload copy parity.
- **R27 � Metadata append cap for progressive grow (16 entries per tick)** (v2.5.1 / 2026-05-08). `PacsClient/pacs/patient_tab/ui/patient_ui/_vc_progressive.py` line 60: `_FAST_PROGRESSIVE_METADATA_APPEND_CAP = 16`. Non-terminal grows call `_refresh_and_sync_metadata(series_number, max_new_entries=_FAST_PROGRESSIVE_METADATA_APPEND_CAP)` to batch metadata updates in 16-slice increments. Terminal completions pass `terminal=True` and do full unbounded sync (no cap). This reduces metadata update storms from 1-per-150ms to 1-per-700ms (see R28 throttle gate). **Why 16?** Series with 100 slices grow in 6-7 ticks to completion; balances responsiveness (~50ms sync work per tick) with frequency. Do NOT increase cap above 32 without measuring latency impact. Do NOT pass cap to terminal path � completions must sync all downloaded slices. Tests: `tests/viewer/test_fast_viewer_pipeline.py::TestProgressiveGrow::test_progressive_nonterm_metadata_batched_on_grow`. Regression signal: progressive_grow_apply_ms_p95 > 50 in KPI parse.
- **R28 � Metadata sync interval throttle (700ms minimum between deferred syncs)** (v2.5.1 / 2026-05-08). `_vc_progressive.py` line 57: `_FAST_PROGRESSIVE_METADATA_SYNC_MIN_INTERVAL_MS = 700.0`. Non-terminal grows run every 150ms and may attempt deferred metadata sync. Gate at line ~1775 checks: `if _last_ms >= 0.0 and (_now_ms - _last_ms) < _FAST_PROGRESSIVE_METADATA_SYNC_MIN_INTERVAL_MS: return`. This allows 4-5 grow ticks to batch between syncs (82% reduction in sync operations). Terminal path always runs sync immediately (no throttle). **Why 700ms?** 700 = 4.67 � 150ms, giving good batching without user-perceivable staleness. **Why separate from R27 cap?** Metadata append cap (R27) limits per-sync update size; interval throttle (R28) gates sync frequency. Both together prevent rapid re-entry storms. Do NOT lower throttle below 500ms or increase above 2000ms without KPI validation. Check condition at line ~1775 � if missing, will regress to 1-per-150ms sync storm. Tests: `tests/viewer/test_fast_viewer_pipeline.py::TestProgressiveGrow::test_progressive_metadata_sync_throttle`. Regression signal: metadata sync count 10x higher than disk scan count per session.
- **R27+R28 combined effect:** progressive_grow_apply_ms_p95 optimized from 200-3000ms (pre-fix) to 2-4ms (post-fix) = 50-1000x improvement. Log baseline: `docs/plans/FAST_VIEWER_STABILIZATION_2026-05-08.md`. Safeguards and monitoring rules: `docs/plans/FAST_VIEWER_REGRESSION_GUARDS_2026-05-08.md`.
- **R29 � Advanced VTK partial-stack loading invariants (2026-05-16 / forensic cleanup)**. Root cause of "series shows only N_partial/N_total slices in Advanced viewer": a three-node chain � (C-A) `_select_dominant_size_dicom_files()` in `image_io.py` reduces `dicom_files` from N_db to N_filtered when a series has mixed image dimensions, but `metadata['instances']` is never updated after the reduction; (C-B) `_get_cached_metadata()` used cache key `"series_{pk}"` with no file-count component, so a stale full-series 20-instance entry was returned on a subsequent partial 8-instance load; (C-C) `get_count_of_slices()` returned `max(range, dims, meta)`, letting `meta_count=20` beat `vtk_count=8` ? slider range [0,19], K-flip mapped lower display positions to raw_k values beyond the actual VTK Z extent, VTK clamped ? frozen image for the lower half of the stack. Plus (Fix 4) `reset_slider()` passed display_k (`mid_slices`) to `apply_default_window_level()` which indexes `metadata['instances'][raw_k]`, so the wrong slice WL preset poisoned the cache at series switch. **Four load-bearing fixes � DO NOT REVERT:** (1) `get_count_of_slices()` in `modules/viewer/advanced/viewer_2d.py` � VTK-derived count (`max(range_count, dims_count)`) is authoritative when `> 1`; `meta_count` is only consulted as last-resort fallback for an uninitialized viewer (`vtk_count ? 1`). Do NOT reintroduce `max(range, dims, meta)` � the metadata path is the stale-data attack surface. Plugin copy at `builder/plugin package/�/viewer_2d.py` MUST stay SHA-equal (SHA: `9157AE5847625D1042E796AA29F8F6B7E82100F6F0B128A2DB53A43391A992B9`). (2) Metadata clip in `image_io.py` immediately after `_select_dominant_size_dicom_files`: if `len(dicom_files) != len(metadata['instances'])`, filter instances by path-set intersection (case-insensitive) then fall back to index truncation; log `[PARTIAL_STACK_CLIP]` at WARNING with `component=viewer`. Do NOT remove this block � without it, C-A guarantees a (vtk_8, meta_20) mismatch that fix (1) merely masks at the `get_count_of_slices` level but does not fix at the source. (3) Cache key in `_get_cached_metadata()` is `f"series_{series_pk}_n{len(instances)}"`. Do NOT revert to `"series_{pk}"` � the count-free key silently re-routes partial loads to a full-series stale entry, defeating fix (2) on any second visit. (4) `reset_slider()` in `_pw_viewers.py` calls `apply_default_window_level(vtk_widget.image_viewer.GetSlice())` � raw_k from the viewer, NOT `mid_slices` (display_k). Do NOT pass any display-domain value to `apply_default_window_level` � it indexes `metadata['instances']` in the raw_k domain. Diagnostic signature of regression: "series shows only 8 of 20 slices; scrolling below slice 9 shows a frozen image" � this is C-C still present; `[PARTIAL_STACK_CLIP]` absent from logs when C-A fires � this is fix (2) missing; cache hit for wrong N in verbose meta_cache logs � this is fix (3) missing. Tests: `tests/viewer/test_partial_stack_loading.py` (18 tests � Fix 1 authoritative count 6 tests, Fix 2 clip logic 5 tests, Fix 3 cache key isolation 4 tests, Fix 4 WL domain 3 tests) + `tests/viewer/test_fast_viewer_reset_slider.py` (updated, 4 tests � Fix 4 contracts including new `test_reset_slider_wl_receives_raw_k_not_display_k` and `test_reset_slider_wl_not_called_for_qt_bridge`).
- **R30 � DisplayGeometry 1-based to 0-based index transformation initialization (2026-05-17 / critical global fix)**. Root cause of "slice 1 shows INFERIOR instead of SUPERIOR" global numbering inversion affecting ALL anatomical planes: the `_display_to_raw_ijk` matrix transformation in `modules/viewer/geometry/display_geometry.py` was initialized as pure identity without the critical -1 offset needed to convert 1-based display indices [1..N] to 0-based raw VTK indices [0..N-1]. **Design contract:** DisplayGeometry uses 1-based display policy (user sees "Slice 1", "Slice 2", ..., "Slice N"), while VTK renderer expects 0-based indices [0..N-1]. Transformation: `raw_k = display_k - 1`. **Bug:** Initialization used `_mat4_identity()` which returns identity matrix (M[2,3]=0.0), giving `raw_k = display_k` instead of `raw_k = display_k - 1`. This broke the counter formula `display_slice = max(0, raw_k_to_display_k(raw_k) - 1)` producing duplicate counters (raw_k=0?counter=1, raw_k=1?counter=1, instead of counter=1,2). **Fix:** In `DisplayGeometry.__init__()` (line 200) and `reset()` (line 213), initialize the transformation with the -1 offset: `self._display_to_raw_ijk[2, 3] = -1.0` IMMEDIATELY after creating the identity matrix. This ensures `raw_k_to_display_k(raw_k) = raw_k + 1` (1-based), and the counter formula produces correct numbering [1..N]. **Files:** (1) `modules/viewer/geometry/display_geometry.py` (canonical), (2) `builder/plugin package/packages/viewer/payload/python/modules/viewer/geometry/display_geometry.py` (plugin mirror � MUST maintain SHA parity). Do NOT remove the offset or initialize the matrix as pure identity � it reintroduces the global inversion. The offset implements the fundamental 1-based display policy contract. Tests verify: raw_k=0?counter=1 (SUPERIOR), raw_k=19?counter=20 (INFERIOR), all intermediate values correct. No changes needed to counter formula, get_display_slice(), or any other code path � the matrix fix alone resolves all anatomical plane inversions globally.
- **R31 � Advanced VTK stack-order contract must stay pixel/metadata aligned (2026-05-17 / critical follow-up to R30)**. Root cause of the "nothing changed" post-fix regression: the display-side R31 logic existed, but `process_series_groups()` in `PacsClient/pacs/patient_tab/utils/image_io.py` still built the ITK volume from the pre-geometry `files` list while `viewer_2d.py` was reading `_geometry_index_applied_reverse` from `metadata['series']` instead of the root metadata dict. Result: the viewer could receive partially-correct metadata while still rendering the old stack order, so numbering remained visibly reversed. **Canonical fix set:** (1) every Advanced load branch must pass `geometry_index.dicom_files_for_itk` into SimpleITK before volume creation; (2) every Advanced load branch must stamp the yielded metadata with that same geometry index; (3) `viewer_2d.py` must read `_geometry_index_applied_reverse` from the root metadata dict and pass it to `DisplayGeometry.audit_stack_order_convention(...)`; (4) geometry-metadata propagation is centralized in `image_io._apply_geometry_index_metadata(...)` so DB/filesystem/process-groups paths cannot drift. Do NOT reintroduce branch-local geometry metadata writes that bypass the shared helper. Do NOT let any Advanced path build pixels from one ordering authority and metadata from another. Regression signal: direct loader probe shows `display_convention='AXIAL_LIKE_EXTREMITY'` and `applied_reverse=True`, but the live viewer still shows unchanged numbering � that means the volume-build path and metadata-stamp path diverged again. Guard test: `tests/viewer/test_canonical_series_sort.py::TestSeriesGeometryIndexContract::test_apply_geometry_index_metadata_populates_series_contract`.
- **R32 � Eagle Eye AI MG module: executor future MUST be stored + done_callback MUST discard `_ai_inflight` on failure (2026-05-20 / critical AI box regression fix)**. File: `modules/ai_imaging/ai_module_ui/overrides/vtk_widget.py` � class `AIVTKWidget`. **Root cause of "AI bounding box never appears on series" (confirmed 2026-05-20):** `_prefetch_worker` submitted `_compute_boxes_scores_for_metadata` via `executor.submit(...)` without storing the returned `Future` and without any exception handler. Any Python exception raised inside the submitted function was permanently and silently swallowed by the `ThreadPoolExecutor`. Secondary effect: `_compute_boxes_scores_for_metadata` raises BEFORE `_ai_inflight.discard(cache_key)` ? `cache_key` stuck in `_ai_inflight` forever. Every subsequent `manager_ai` call finds `cache_key in _ai_inflight` ? returns early with no work done. Box NEVER displays until app restart. **Canonical fix (do NOT revert):** (1) Store the future: `future = executor.submit(self._compute_boxes_scores_for_metadata, ...)`. (2) Attach a `_on_prefetch_done` done_callback with loop-variable capture via default args (`_key=cache_key, _uid=series_uid`): `future.add_done_callback(_on_prefetch_done)`. (3) Inside `_on_prefetch_done`: call `f.exception()` � if not None, log `[MG][PREFETCH_WORKER_ERROR] series_uid=... err=... tb=...` at WARNING with `extra={"component": "viewer"}` + call `self._ai_inflight.discard(_key)` inside `self._ai_cache_lock`. This ensures `_ai_inflight` is ALWAYS cleaned up on both success (done by the `with self._ai_cache_lock:` block at end of `_compute_boxes_scores_for_metadata`) and failure (done by `_on_prefetch_done`). **`_ai_inflight` invariant:** `_ai_inflight.discard(cache_key)` MUST be called in ALL exit paths from `_compute_boxes_scores_for_metadata` � both the normal success path (`with self._ai_cache_lock:` block before `AUTO_APPLY`) and the failure path (done_callback). If this invariant is broken, all future AI-box attempts for that series are silently blocked until app restart. **Do NOT access `self.image_viewer`, `self.image_viewer.metadata`, or any Qt/VTK object from inside `_compute_boxes_scores_for_metadata` or any function it calls.** These are Qt-thread-owned objects; accessing them from a background `ThreadPoolExecutor` thread causes non-Python crashes on Windows (access violation). The UID-mismatch check using `self.image_viewer.metadata.get(...)` was removed from `_compute_boxes_scores_for_metadata` for this reason � cross-thread VTK access is architecturally unsafe even inside `try/except`. The `_schedule_apply_boxes_safe` call at the end of `_compute_boxes_scores_for_metadata` uses Qt signal dispatch (thread-safe) and is the correct mechanism for returning to the Qt thread. **Score formatting hardening:** The `float(f'{score_value:.2f}')` call in the for-loop inside `_compute_boxes_scores_for_metadata` MUST be wrapped in `try/except (TypeError, ValueError)` with a fallback chain `round(float(score_value), 2)` ? `'Custom'`. A non-numeric `score_value` (e.g. `None`, a string, an empty list from CSV parse) would otherwise raise `TypeError: unsupported format character` which silently kills the entire executor task via the swallow-exception behavior above. **Step diagnostic logs (do NOT remove):** Three `[MG][COMPUTE_STEP]` WARNING log lines in `_compute_boxes_scores_for_metadata` � `step=before_for_loop`, `step=after_for_loop`, `step=cache_stored` � are the primary production diagnostic for pinpointing where the pipeline stalls. If `[MG][GEOM_CONVERT]` fires but `step=before_for_loop` never appears in `viewer_diagnostics.log`, the crash is in the `if new_boxes:` or score-length guard blocks between GEOM_CONVERT and the for-loop. If `step=before_for_loop` fires but `step=after_for_loop` does not, the crash is inside the for-loop. **Shared state:** `_ai_boxes_cache`, `_ai_cache_lock`, and `_ai_inflight` are all shared from `patient_widget` attributes when `patient_widget is not None` (see `__init__`). All access to these MUST be done under `_ai_cache_lock`. `patient_widget._ai_prefetch_executor` (ThreadPoolExecutor) is also shared across all `AIVTKWidget` instances for the same patient. Do NOT create a per-viewer executor � it defeats the shared-inflight deduplication. **`_ai_prefetch_started` guard:** `_ensure_ai_prefetch_for_all_series` returns immediately if `patient_widget._ai_prefetch_started` is already `True` � this prevents duplicate prefetch threads per patient. Do NOT reset this flag inside `_compute_boxes_scores_for_metadata` or on series switch. Regression signal: "AI bounding box does not appear on any MG series" ? check `viewer_diagnostics.log` for `[MG][PREFETCH_WORKER_ERROR]` and verify `[MG][AUTO_APPLY_FROM_PREFETCH]` fires. If neither appears but `[MG][GEOM_CONVERT]` fires 5� per app run, the executor exception is being silently swallowed again.
...
- **Local backup of this stable version:** `backups/v2.4.8c_conservative_2026-05-03.zip` (and prior `backups/v2.4.7c_conservative_2026-05-02.zip`)
...
### Branch lock for this workstation (Matab PC)
- **This project instance MUST stay on the `matab-conservative` branch.** Other contributors work on other branches (e.g., `main`); do NOT switch this clone to `main` or merge `main` into the working tree without an explicit instruction from the user.
- **Sync rule:** `git fetch origin && git pull --ff-only origin matab-conservative` only. Never run a non-fast-forward merge or rebase from another branch on this machine.
- **Build rule:** all PyInstaller and Nuitka release builds produced on this PC are built from `matab-conservative` HEAD. Verify with `git branch --show-current` before invoking `build.py` or `builder nuitka/build_nuitka_release.py`.
- **If `git branch --show-current` ever reports anything other than `matab-conservative` on this clone, stop and ask the user before performing any further build, push, or sync action.**
...
| `modules/ai_imaging/` | `AIVTKWidget` (`overrides/vtk_widget.py`) | AI segmentation and analysis UI; Eagle Eye MG mammography AI box detection, display, CSV persistence, sidebar seeding � see R32 for critical invariants |
...
| Priority-handoff KPI parser (F3.5.1) | `tests/performance/test_priority_handoff_kpi_parser.py` | 15 tests � round-trip per tag, branch field, diagnostic_logging prefix tolerance, p50/p95 aggregation | `python -m pytest tests/performance/test_priority_handoff_kpi_parser.py -v` |
| Priority-handoff instrumentation (F3.5.1) | `tests/download_manager/test_priority_handoff_instrumentation.py` | 7 tests � begin?started chain, started_ms lifecycle, exhaust branch=recovery, recover branch=primary, trace gate, format stability, dedup | `python -m pytest tests/download_manager/test_priority_handoff_instrumentation.py -v` |
