...
- **Local backup of this stable version:** `backups/v2.3.5_2026-04-19/`
...
- **Untargeted progressive first-display defers once viewers are already occupied** (2026-04-16). `_start_progressive_display` now also defers when `target_vtk_widget is None`, `_first_series_displayed` is already true, and there is no empty viewer to fill. This prevents a newly downloading background series from stealing a 1s+ first-load/rebind on the active layout while the user is still reading another series. Do NOT remove this guard unless a viewer explicitly requested the series or a genuinely empty viewer must be filled.
- **Untargeted progressive defer is sticky until layout eligibility changes** (2026-04-16, same-day hardening). Once an untargeted background series is deferred because there is no awaiting or empty viewer, `on_series_images_progress` must NOT keep re-entering `_start_progressive_display` on every later progress pulse. Keep the series in `_progressive_untargeted_defer` and retry only when a viewer becomes empty, a viewer explicitly awaits the series, or the first-series gate is no longer active. Do NOT remove this guard-set — repeated untargeted retries reintroduced layout CPU churn without improving UX.
