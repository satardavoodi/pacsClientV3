"""DICOMDIR builder — standards-compliant DICOM Media Storage Directory.

Single implementation shared by BOTH media workflows (do NOT fork):

* the **CD burner** (`modules/cd_burner`, an optional plugin), and
* **Offline Sync** export (`PacsClient/utils/offline_cloud.py`, core).

It lives in a CORE module because `modules.cd_burner` is excluded from the
engine (it only ships inside the optional `run_cd` plugin payload), so core code
cannot import from it. `modules/cd_burner/dicomdir_builder.py` is a thin
re-export shim onto this module.

Why the standard PT/ST/SE/IM tree (and not "DICOMDIR in place"): a DICOM File ID
component must be <= 8 chars from [A-Z0-9_] (PS3.10). Real folder names such as a
StudyInstanceUID (`1.2.840.113619...`) can NEVER be a valid File ID, so a
compliant DICOMDIR cannot reference an arbitrary layout. pydicom's
``FileSet.write()`` lays out the compliant tree and writes DICOMDIR with valid
relative File IDs.
"""
...
# defaults; UIDs and pixel data are never touched.
...
def _ensure_dicomdir_fields(ds) -> None:
    ...
    # PatientName is Type-2 in the PATIENT record — empty allowed, must exist.
...
class DicomDirBuilder:
    """Builds a standards-compliant DICOMDIR (+ PT/ST/SE/IM tree) from DICOM files.

    After a build, ``last_stats`` holds the counts required for logging /
    validation reporting by the caller.
    """

    def __init__(self):
        self.progress_callback: Optional[Callable[[int, str], None]] = None
        self.last_stats: Dict[str, object] = _new_stats()
    ...
    def _report_progress(self, percent: int, message: str):
        ...
        logger.info("[DICOMDIR %d%%] %s", percent, message)
    ...
    def build_from_study_folders(
        ...
        """Build DICOMDIR (and the compliant file tree) at ``output_folder``.

        Returns True on success. Detailed counts land in ``self.last_stats``.
        """
        self.last_stats = _new_stats()
        ...
        try:
            ...
            for study_folder in study_folders:
                ...
                if study_path.exists():
                    all_dicom_files.extend(self._find_dicom_files(study_path))

            stats["files_found"] = len(all_dicom_files)
            ...
            for dcm_path in all_dicom_files:
                try:
                    ...
                    hierarchy[patient_key]["studies"][study_uid][series_uid]["files"].append(
                        {"path": dcm_path, "instance": instance_num, "dataset": ds}
                    )

                except Exception as e:
                    logger.warning(f"Error reading {dcm_path}: {e}")
                    stats["unreadable"] = int(stats["unreadable"]) + 1
                    continue
            ...
            stats["patients"] = len(hierarchy)
            stats["studies"] = sum(len(p["studies"]) for p in hierarchy.values())
            stats["series"] = sum(
                len(series_dict)
                for p in hierarchy.values()
                for series_dict in p["studies"].values()
            )

            self._report_progress(50, "Adding files to DICOMDIR...")

            expected_sop_instance_uids: set = set()
            add_failures: list = []
                    ...
                    for series_uid, series_data in series_dict.items():
                        for file_info in series_data["files"]:
                            try:
                                ...
                                # Never write duplicate SOP Instance UID entries.
                                if sop_uid and sop_uid in expected_sop_instance_uids:
                                    stats["duplicates_skipped"] = int(stats["duplicates_skipped"]) + 1
                                    logger.warning(
                                        "DICOMDIR: duplicate SOPInstanceUID skipped (%s): %s",
            ...
            stats["instances_added"] = len(expected_sop_instance_uids)
            stats["failed"] = len(add_failures)
            stats["failures"] = add_failures[:20]

            # A File-set with ZERO instances is a silent failure: it writes a
            # 0-image DICOMDIR that used to "pass" validation (empty == empty)
            # and produced media with no images. Fail loudly instead.
            ...
            # (loading one triggers the deprecated DicomDir class in pydicom v3).
            ...
            stats["dicomdir_path"] = str(output_path / "DICOMDIR")
            stats["ok"] = True
            self._report_progress(100, "DICOMDIR created successfully")
            logger.info(
                "DICOMDIR created at %s (patients=%s studies=%s series=%s instances=%s)",
                stats["dicomdir_path"], stats["patients"], stats["studies"],
                stats["series"], stats["instances_added"],
            )
            return True

        except Exception as e:
            logger.error(f"Error building DICOMDIR: {e}")
            logger.exception("DICOMDIR build failed")
            return False

    def _find_dicom_files(self, study_path: Path) -> List[Path]:
        """Return DICOM files under ``study_path``.

        Prefers common DICOM suffixes and falls back to extension-less files
        (patient media often stores instances with no extension).
        """
        matches = []
    ...
    def _validate_output_fileset(self, output_path: Path, expected_uids: set) -> bool:
        """DICOMDIR exists, references exactly the expected instances, and every
        referenced file is present on disk."""
        try:
            ...
            # A 0-instance File-set must never validate as success.
            ...
            if actual_uids != expected_uids:
                ...
                logger.error(
                    ...
                    missing, extra, len(expected_uids), len(actual_uids),
                )
                return False
            ...
            for instance in validation_fs:
                ...
                if not instance_path.exists():
                    logger.error("Referenced file missing from File-set: %s", instance_path)
                    return False
