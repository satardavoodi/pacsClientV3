...
def write_lesion_report(result, flair, mask, directory):
    ...
    pages = [first] + svd_pages(result) + ms_pages(result) + comparison_pages(result)
    for index, path, height in previews:
        ...
        pages.append('<h1>Native FLAIR mask review</h1>'
                     f'<p>Native slice index: {index} (zero-based). Red: lesion candidates.</p>'
                     f'<p><img src="{escape(path.as_uri(), quote=True)}" width="{display_width}"></p>'
