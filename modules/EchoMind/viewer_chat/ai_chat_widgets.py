...
class UnifiedComposer(QWidget):
    ...
    def __init__(self, placeholder: str = "Write/paste report text"):
        ...
        _set_icon(self.btn_pause, "pause.png", 18, "Pause/Resume Recording")
        ...
        self.btn_pause.setStyleSheet("""
            ...
            QToolButton:hover { background: #d35400; }
            QToolButton:pressed { background: #ba4a00; }
        """)
        self.btn_pause.clicked.connect(self._toggle_pause_record)

        # ✅ دکمه تایید ضبط (تیک سبز)
        self.btn_confirm_rec = QToolButton(controls)
        self.btn_confirm_rec.setToolButtonStyle(Qt.ToolButtonIconOnly)
        self.btn_confirm_rec.setCursor(Qt.PointingHandCursor)
        self.btn_confirm_rec.setIconSize(QSize(18, 18))
        self.btn_confirm_rec.setFixedSize(34, 34)
        _set_icon(self.btn_confirm_rec, "check.png", 18, "Finish & Transcribe")
        self.btn_confirm_rec.setVisible(False)
        self.btn_confirm_rec.setStyleSheet("""
            QToolButton {
                background: #27ae60;
                border: none;
                border-radius: 17px;
            }
            QToolButton:hover { background: #1e8449; }
            QToolButton:pressed { background: #196f3d; }
        """)
        self.btn_confirm_rec.clicked.connect(self._on_confirm_record_clicked)

        # ❌ دکمه کنسل (ضربدر قرمز)
        ...
        _set_icon(self.btn_cancel, "x.png", 18, "Cancel Recording")
        ...
        self.btn_cancel.setStyleSheet("""
            ...
            QToolButton:pressed { background: #a93226; }
        """)
        self.btn_cancel.clicked.connect(self._on_cancel_clicked)

        # Apply initial mic mode now that all recording buttons exist
        ...
        # 🤖 دکمه Assist
        self.btn_assist_send = QToolButton(controls)
        self.btn_assist_send.setToolButtonStyle(Qt.ToolButtonTextBesideIcon)
        self.btn_assist_send.setCursor(Qt.PointingHandCursor)
        self.btn_assist_send.setIconSize(QSize(18, 18))
        _set_icon(self.btn_assist_send, "assistant.png", 18, "Run Assistant")
        self.btn_assist_send.setText(" Assist")
        self.btn_assist_send.setFixedHeight(36)
        self.btn_assist_send.setMinimumWidth(80)
        ...
        # 🔍 دکمه Search
        self.btn_search_send = QToolButton(controls)
        self.btn_search_send.setToolButtonStyle(Qt.ToolButtonTextBesideIcon)
        self.btn_search_send.setCursor(Qt.PointingHandCursor)
        self.btn_search_send.setIconSize(QSize(18, 18))
        _set_icon(self.btn_search_send, "search.png", 18, "Run Search")
        self.btn_search_send.setText(" Search")
        self.btn_search_send.setFixedHeight(36)
        self.btn_search_send.setMinimumWidth(80)
    ...
    def _apply_pause_icon(self, is_paused: bool):
        if is_paused:
            _set_icon(self.btn_pause, "play.png", 18, "Resume recording")
            self.btn_pause.setToolTip("Resume recording")
        else:
            _set_icon(self.btn_pause, "pause.png", 18, "Pause recording")
    ...
    def _emit_assist(self):
        """Emit assistClicked signal with current text."""
        txt = self.box.toPlainText().strip()
        self.assistClicked.emit(txt)

    def _emit_search(self):
        """Emit searchClicked signal with current text."""
        txt = self.box.toPlainText().strip()
        self.searchClicked.emit(txt)

    def _on_confirm_record_clicked(self):
        """Finish recording and transcribe (same as confirm mic click)."""
    ...
    def add_voice_attachment(self, path: str):
        ...
        # After finishing recording, restore mic UI
        self._restore_mic_after_record()
    ...
    def _apply_mic_mode(self, mode: str):
        ...
        if mode == "record":
            ...
            # Hide mic button itself, show the 3 recording control buttons
            self.btn_mic.setVisible(False)
            self.btn_pause.setVisible(True)
            self.btn_confirm_rec.setVisible(True)
    ...
    def _on_cancel_clicked(self):
        if self._rec_running:
            ...
            self._apply_mic_mode("record")
            self.btn_mic.setVisible(True)  # ✅ restore mic button
        else:
    ...
    def _restore_mic_after_record(self):
        """Restore mic button visibility and hide recording control buttons."""
