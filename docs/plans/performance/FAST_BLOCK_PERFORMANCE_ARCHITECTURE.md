...
- `modules/viewer/fast/block_telemetry.py`
   ...
   - compact `[BLOCK_DIAG]` and KPI-focused `[BLOCK_KPI]` / `[BLOCK_KPI_JSON]` heartbeat payloads for live logs
...
- log prefixes:
   - `[BLOCK_DIAG]` — machine-friendly compact overlap/control snapshot
   - `[BLOCK_KPI_JSON]` — machine-friendly per-block KPI payload
   - `[BLOCK_KPI]` — human-readable per-block KPI summary for live workstation runs
...
- what the key per-block KPIs are at that same moment without needing to post-process the full snapshot offline
