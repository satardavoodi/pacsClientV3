...
### Optional block-oriented summary after normalization

Once a KPI JSON exists for either AI-PACS or ClearCanvas, you can also group the results by the FAST block model:

```powershell
.venv\Scripts\python.exe tools\performance\clearcanvas_aipacs_kpi_harness.py summarize-blocks `
  --payload generated-files\benchmarks\run_001\aipacs_common.json `
  --output generated-files\benchmarks\run_001\aipacs_common_blocks.json `
  --markdown-output generated-files\benchmarks\run_001\aipacs_common_blocks.md
```

This groups the run into:

- Block 1 — data services
- Block 2 — viewer hot path
- Block 3 — cache/scroll/orchestration

and shows which KPIs were present vs still missing for each block.
