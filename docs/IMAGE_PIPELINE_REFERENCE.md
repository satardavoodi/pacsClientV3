...
>
> **Audience**: All AIPacs developers — especially those working on sync/target,
...
2. [Stage 1 — DICOM on Disk](#2-stage-1--dicom-on-disk)
3. [Stage 2 — SimpleITK Loading](#3-stage-2--simpleitk-loading)
4. [Stage 3 — ITK → VTK Conversion (`convert_itk2vtk`)](#4-stage-3--itk--vtk-conversion)
5. [Stage 4 — Viewer Preprocessing](#5-stage-4--viewer-preprocessing)
6. [Stage 5 — ImageReslice & Display](#6-stage-5--imagereslice--display)
7. [Stage 6 — MPR Path (StandardMPRViewer)](#7-stage-6--mpr-path)
...
10. [Direction Matrix — What It Is, How It Changes](#10-direction-matrix)
...
```
DICOM files ──► SimpleITK (CS-1/CS-2) ──► convert_itk2vtk ──► CS-3 ──► reslice ──► CS-4 ──► display ──► CS-5
                                              │
                                              ▼
                                        Y-flip + direction
                                        matrix compensation
...
### 2026-02-15 Update — ZetaBoost multi-lane preload + L2 persistent cache

To improve high-volume studies (e.g., 1000–3000 instances/series), the preload system now uses:
...
The third direction (slice/normal) is the cross product: `normal = row × column`
...
2. It builds a 3D volume with:
   ...
   - **Direction** = a 3×3 matrix built from `ImageOrientationPatient` + cross product
...
## 3. Stage 2 — SimpleITK Loading

### Code Location
- `PacsClient/pacs/patient_tab/utils/image_io.py` → `get_itk_image()`
- `PacsClient/pacs/patient_tab/utils/utils.py` → `get_itk_image_optimized()`
...
|----------|--------|-------------|
| Size | `GetSize()` → `(x, y, z)` | Number of voxels along each axis. **Note**: X=columns, Y=rows, Z=slices |
| Origin | `GetOrigin()` → `(ox, oy, oz)` | Position of voxel (0,0,0) in LPS+ patient mm |
| Spacing | `GetSpacing()` → `(sx, sy, sz)` | Voxel size in mm along each axis |
| Direction | `GetDirection()` → 9 floats | 3×3 direction cosine matrix, **row-major** |
...
```
direction = (d00, d01, d02,   ← row 0: how voxel-X maps to patient-XYZ
             d10, d11, d12,   ← row 1: how voxel-Y maps to patient-XYZ
             d20, d21, d22)   ← row 2: how voxel-Z maps to patient-XYZ
```
...
```
⎡ px ⎤   ⎡ ox ⎤   ⎡ d00  d01  d02 ⎤   ⎡ i·sx ⎤
⎢ py ⎥ = ⎢ oy ⎥ + ⎢ d10  d11  d12 ⎥ · ⎢ j·sy ⎥
⎣ pz ⎦   ⎣ oz ⎦   ⎣ d20  d21  d22 ⎦   ⎣ k·sz ⎦
```
...
**This is the ZYX convention** — SimpleITK stores arrays with Z (slices) as the
slowest-varying axis and X (columns) as the fastest.

---

## 4. Stage 3 — ITK → VTK Conversion

### Code Location
- `PacsClient/pacs/patient_tab/utils/utils.py` → `convert_itk2vtk()`
...
**Step 2**: Build a 4×4 direction matrix from the ITK 3×3 direction.
At this point, the matrix is the **original ITK direction**.

```python
    arr = sitk.GetArrayFromImage(itk_image)  # shape: (z, y, x)
    arr = arr[:, ::-1, :]                     # ← THE Y-FLIP
```

**Step 3 — THE Y-FLIP**: The numpy array is flipped along axis-1 (the Y/row axis).
...
**Step 4 — Direction Matrix Compensation**: Row 1 of the direction matrix is negated.

This is the mathematical compensation for the Y-flip:
- Original: `patient = origin + D @ [i·sx, j·sy, k·sz]ᵀ`
...
```
D_stored = ⎡  d00   d01   d02 ⎤
           ⎢ -d10  -d11  -d12 ⎥   ← row 1 negated
           ⎣  d20   d21   d22 ⎦
```
...
`"DirectionMatrix"` — 16 doubles (4×4 matrix, row-major).
...
| Origin | Same as ITK `GetOrigin()` | Unchanged — **but the Y-flip means the origin no longer points to the physical location of voxel (0,0,0)!** |
| Pixel data | Y-flipped | `arr[:, ::-1, :]` |
| Field data `"DirectionMatrix"` | Row-1 negated 4×4 | Compensates for Y-flip |
| Field data `"ITKOrigin"` | 3 doubles | Original ITK origin (same as VTK origin) |
| Field data `"ITKSpacing"` | 3 doubles | **Original ITK spacing** (critical — differs from VTK after upsampling) |
| Field data `"ITKDimensions"` | 3 doubles | **Original ITK dimensions** (critical — differs from VTK after upsampling) |
...
- ⚠️ **Upsampling changes spacing and dimensions!** After upsampling, the VTK
  image's `GetSpacing()` and `GetDimensions()` no longer match the original ITK values.
  For example, a 580×640 image with 0.3438mm spacing may become 753×831 with 0.264mm spacing.
...
class ImageReslice(vtk.vtkImageReslice):
    ...
    def apply_orientation(self):
        pass    # ← NO-OP!  No rotation is applied!
```

- **The reslice is a pass-through** — it does not rotate or reorient
...
|----------|------------------|----------------|
| Field data `"DirectionMatrix"` | ✅ Present | ❌ **Lost** (VTK filters don't propagate field data) |
| `SetDirectionMatrix()` | ✅ Set (if VTK 9+) | ❌ **Not propagated** by `vtkImageReslice` |
...
- `self.vtk_image_data` → reslice output (no direction, no field data)
- `self.image_reslice.vtk_image_data` → original input (has direction in field data)
...
## 6. Stage 5 — ImageReslice & Display
...
"simple" VTK world space (origin + ijk × spacing).  These are **NOT** patient
coordinates — they don't account for the direction matrix.
...
└──────────────────────┬──────────────────────────────────┘
                       ...
                       ▼
┌─────────────────────────────────────────────────────────┐
│  Stage 1: SimpleITK ImageSeriesReader                    │
│  ─────────────────────────────────────                   │
│  Reads and assembles DICOM into 3D volume                │
│  ✓ Origin preserved (from first slice IPP)               │
│  ✓ Spacing preserved                                     │
│  ✓ Direction = 3×3 from ImageOrientationPatient          │
│  ✓ Numpy: shape = (z, y, x) — ZYX order                 │
│  ✗ NO reorientation applied                              │
│  ✗ NO DICOMOrient called                                 │
│                                                          │
│  Formula: patient = origin + D_itk @ (ijk * spacing)    │
└──────────────────────┬──────────────────────────────────┘
                       │
                       ▼
┌─────────────────────────────────────────────────────────┐
...
│  ──────────────────────────                               │
│  1. SetDimensions(x, y, z)  — same as ITK               │
│  2. SetSpacing(itk.GetSpacing())  — unchanged            │
│  3. SetOrigin(itk.GetOrigin())  — unchanged              │
│  4. Build D_itk from GetDirection()                      │
│  5. *** Y-FLIP: arr[:, ::-1, :] ***                      │
│     Voxel (i, j, k) → (i, y-1-j, k)                    │
│  6. Negate row 1 of D → D_stored                        │
│     D_stored[1,:] = -D_itk[1,:]                         │
│  7. Store D_stored as FieldData "DirectionMatrix"        │
│                                                          │
│  Output has: Y-flipped pixels, original origin,          │
│  compensated direction in field data                     │
└──────────────────────┬──────────────────────────────────┘
                       │
           ┌───────────┴───────────┐
           ▼                       ▼
┌─────────────────────┐ ┌──────────────────────┐
│  2D Viewer Path     │ │  MPR Path             │
│  (ImageViewer2D)    │ │  (StandardMPRViewer)  │
└────────┬────────────┘ └────────┬─────────────┘
         │                       │
         ▼                       ▼
┌─────────────────────┐ ┌──────────────────────┐
│ Stage 3a: Viewer    │ │ Stage 3b: MPR        │
│ ──────────────────  │ │ ──────────────────── │
│ 1. Optional         │ │ 1. X-FLIP:           │
│    upsample XY      │ │    vtkImageFlip(0)   │
│ 2. SetDirection     │ │ 2. Negate column 0   │
│    Matrix (VTK9)    │ │    of direction       │
│ 3. ImageReslice     │ │ 3. Camera setup      │
│    (pass-through)   │ │    from direction     │
│ 4. vtk_image_data   │ │ 4. vtkImageReslice   │
│    = reslice output  │ │    Mapper per view   │
│    (direction LOST) │ │                      │
│                     │ │ Has: X+Y flipped     │
│ Has: Y-flipped only│ │ pixels, original     │
│ origin, NO          │ │ origin, doubly-      │
│ direction on output │ │ compensated          │
│                     │ │ direction            │
└────────┬────────────┘ └────────┬─────────────┘
         │                       │
         ▼                       ▼
┌─────────────────────────────────────────────────────────┐
│  Stage 4: Display / Rendering                            │
│  ─────────────────────────────                           │
│  2D: vtkResliceImageViewer slices along XY/YZ/XZ        │
│  MPR: vtkImageResliceMapper with camera-oriented slicing │
│                                                          │
│  Pickers return coords in VTK world space:               │
│    vtk_world = origin + ijk * spacing   (identity dir)   │
│                                                          │
│  These are NOT patient coordinates!                      │
└─────────────────────────────────────────────────────────┘
```
...
These static methods in `patient_widget.py` convert between VTK post‑Y‑flip world
...
> the error was invisible (D²=I).

#### `_read_itk_geometry(viewer)` → dict or None
...
| `extent_y` | float | `(dims_y - 1) * spacing_y` — ITK physical Y extent in mm |
| `extent_y_disp` | float | `(display_dims_y - 1) * display_sp_y` — display physical Y extent |
| `origin` | np.ndarray (3,) | Pre-reslice image origin (= ITK origin) |
| `source` | str | `'field_data'` or `'image_fallback'` — where spacing/dims came from |

> **Why two extent_y values?**  `vtkImageResample` rounds output dimensions, so
> `extent_y_disp ≠ extent_y` (a mismatch of several mm for highly-upsampled images).
...
Math (v1.09.5 — fractional Y-flip, no D_vtk):
...
> the mapping is simply delta → undo-Y-flip → D_itk.  The v1.09.4 D_vtk correction was
...
Returns:
- `vtk_world` — tuple(3) clamped VTK world position
- `ijk_itk_raw` — np.ndarray(3) unclamped ITK voxel indices (for diagnostics)
- `was_outside` — bool True if point fell outside the volume (any ijk off by >0.5)

### Cross-Volume Sync Mapping (v1.09.5)

The sync/target feature uses a **three-tier mapping strategy**:

#### Tier 1 — DICOM IOP/IPP (primary, v1.09.5)

The preferred path uses per-slice DICOM metadata (ImageOrientationPatient,
ImagePositionPatient) — the **exact same functions** as `reference_line.py`.
This guarantees the sync dot lies on the reference line.

```
VTK world (source)
  → display index  (simple: (world - origin) / spacing)
  → flipped-LPS    (IPP + idx * sp * IOP_directions)
  → undo flip-Y    (rl_apply_flip_y_in_plane, self-inverse)
  → patient LPS    (true anatomical coordinates)
  → closest target slice  (dot(P - IPP_0, slice_normal) / slice_spacing)
  → project onto plane    (remove off-plane component)
  → flip-Y for target     (rl_apply_flip_y_in_plane)
  → target index          (rl_lps_to_target_index)
  → VTK world (target)    (origin + index * spacing)
```
...
#### Tier 2 — ITK direction matrix (fallback)

```
VTK world A  →  [centre-of-slice adj]  →  _vtk_world_to_patient(A)  →  patient (LPS+)  →  _patient_to_vtk_world_clamped(B)  →  VTK world B
```
...
#### Tier 3 — Fractional mapping (last resort)
...
- **Different orientations** (Axial T2 ↔ Sagittal T1): direction matrices rotate correctly
- **Oblique acquisitions**: any orientation works because the full 3×3 direction is used
...
| Function | File | Input → Output | Direction-aware? | Notes |
|----------|------|---------------|-----------------|-------|
| `_map_sync_dicom()` | patient_widget.py | CS-4 src → CS-4 tgt | ✅ Yes | **Primary** sync path; uses same functions as reference_line.py |
| `_vtk_world_to_patient()` | patient_widget.py | CS-4 VTK world → CS-1 patient | ✅ Yes | Fractional Y-flip; ITK fallback path |
| `_patient_to_vtk_world_clamped()` | patient_widget.py | CS-1 patient → CS-4 VTK world | ✅ Yes | Clamps in voxel space, fractional Y |
| `_read_itk_geometry()` | patient_widget.py | viewer → geometry dict | ✅ Yes | Returns extent_y, extent_y_disp, origin |
| `ijk_to_world_physical()` | viewer_2d.py | CS-3 voxel → CS-1 patient | ✅ Yes | Uses `_get_direction_matrix()` |
| `world_to_ijk_physical()` | viewer_2d.py | CS-1 patient → CS-3 voxel | ✅ Yes | Tries VTK native, falls back to matrix |
| `_slice_index_from_world()` | viewer_2d.py | CS-4 world → slice index | ❌ No | Simple `(world-origin)/spacing` |
| `pick_world_point()` | viewer_2d.py | CS-5 display → CS-4 world | N/A | Uses VTK pickers |
| `world_to_ijk()` | geometry_utils.py | World → IJK | ✅ Yes | From zeta_sync module |
| `ijk_to_world()` | geometry_utils.py | IJK → World | ✅ Yes | From zeta_sync module |
| `build_ijk_to_world_matrix()` | geometry_utils.py | Builds 4×4 affine | ✅ Yes | `origin + D @ diag(spacing) @ ijk` |
| `world_to_ijk_vtk()` | convertors.py | World → IJK | ✅ Yes | Uses `GetDirectionMatrix()` |

### When to use which

- **2D viewer sync/target**: Use `_vtk_world_to_patient` + `_patient_to_vtk_world` for 
  cross‑volume mapping. Use simple `(world - origin) / spacing` for within‑volume slice
...
```
D_itk = ⎡ 1  0  0 ⎤   ← voxel-X aligns with patient-X (Left)
        ⎢ 0  1  0 ⎥   ← voxel-Y aligns with patient-Y (Posterior)
        ⎣ 0  0  1 ⎦   ← voxel-Z aligns with patient-Z (Superior)
```

For a standard **sagittal** acquisition:
```
D_itk = ⎡ 0  0  -1 ⎤  ← voxel-X aligns with -patient-Z (Inferior)
        ⎢ 1  0   0 ⎥  ← voxel-Y aligns with patient-X (Left)
        ⎣ 0 -1   0 ⎦  ← voxel-Z aligns with -patient-Y (Anterior)
```
...
```
D_stored = ⎡  D_itk[0,0]   D_itk[0,1]   D_itk[0,2] ⎤
           ⎢ -D_itk[1,0]  -D_itk[1,1]  -D_itk[1,2] ⎥   ← row 1 negated
           ⎣  D_itk[2,0]   D_itk[2,1]   D_itk[2,2] ⎦
```

### After MPR X-flip compensation (D_mpr)

```
D_mpr = ⎡ -D_stored[0,0]  D_stored[0,1]  D_stored[0,2] ⎤
        ⎢ -D_stored[1,0]  D_stored[1,1]  D_stored[1,2] ⎥   ← column 0 negated
        ⎣ -D_stored[2,0]  D_stored[2,1]  D_stored[2,2] ⎦
```

### To recover original ITK direction from stored field data

```python
D_stored = read_from_field_data()   # 3×3 from "DirectionMatrix"
D_itk = D_stored.copy()
...
|-------|-------------|---------------|-----------------|
| Pre-convert | `itk_image.GetDirection()` | ✅ | D_itk (original) |
| Post-convert, pre-reslice | Field data `"DirectionMatrix"` | ✅ | D_stored (Y-flip compensated) |
| Post-convert, pre-reslice | `SetDirectionMatrix()` (if called) | ✅ | D_stored |
| Post-reslice (viewer) | `self.vtk_image_data` field data | ❌ | **Lost** |
| Post-reslice (viewer) | `self.image_reslice.vtk_image_data` field data | ✅ | D_stored |
| MPR viewer | `self.direction_matrix` | ✅ | D_mpr (X+Y compensated) |
...
matrix** — not in field data, not via `GetDirectionMatrix()`.  Any code that calls
...
```
patient(vtk_0,0,0) = origin + D_itk @ [0, (y-1)·sy, 0]ᵀ
```
...
When direction matrices are available, **patient‑space mapping is required**
...
VTK slice k — **no re-sorting is needed**.
...
- v1.09.5 added `_sort_metadata_instances()` to re-sort by IPP projection,
  ...
  broke the alignment for any series where instance_number ≠ IPP order.
...
- v1.09.7 removed the reversal but kept the IPP sort, still breaking series
  where instance_number ≠ IPP order.
...
```
ITK image:
  Size:      (512, 512, 100)     — 512 cols, 512 rows, 100 slices
  Origin:    (-250.0, -250.0, -500.0)
  Spacing:   (0.977, 0.977, 2.0)
  Direction: (1, 0, 0, 0, 1, 0, 0, 0, 1)    — identity (standard axial)
```
...
Direction stored: D_stored = [[1,0,0], [0,-1,0], [0,0,1]]
  (row 1 negated: [0,1,0] → [0,-1,0])

Origin: (-250.0, -250.0, -500.0)  — unchanged
```
...
| Term | Definition |
|------|-----------|
| **LPS+** | Left-Posterior-Superior — the DICOM/ITK patient coordinate system |
| **Direction matrix** | 3×3 rotation matrix mapping voxel axes to patient axes |
...
| **Reslice output** | The output of `vtkImageReslice` — has identity direction, no field data |
| **Fractional mapping** | Normalize position as fraction of volume A extent, map to same fraction in volume B |
| **extent_y_itk** | `(dims_itk_y - 1) * spacing_itk_y` — physical Y extent using original ITK geometry |
| **extent_y_disp** | `(display_dims_y - 1) * display_sp_y` — physical Y extent in the (upsampled) viewer |
...
| **Display upsampling** | `display_upsample_xy()` — increases XY resolution for display quality; changes spacing/dims |
...
1. **Do not assume direction matrices survive reslice.** If you need direction,
   use the pre‑reslice image (`viewer.image_reslice.vtk_image_data`) or the
   field‑data `DirectionMatrix`.
...
6. **Non-overlapping volumes are expected in MRI.** Different MRI series (e.g. Axial T2
   ...
   warning is correct behavior — not a bug.
...
7. **VTK picker does NOT apply SetDirectionMatrix() rotation.**
   ...
   VTK direction matrix before applying D_itk — that was the v1.09.4 bug.
...
8. **metadata['instances'] stays in DB (instance_number) order.**  VTK slices are
   ...
   instance_number order.  Therefore `metadata['instances'][k]` naturally matches
   VTK slice k with no re-sorting.  **Do NOT re-sort metadata by IPP** — that was
...
### 2026-02-07 — First live test with pipeline tracing logs

#### CT Test: Shoulder CT — Series 201 (3.75mm) ↔ Series 202 (0.625mm)
...
- Post-upsampled dims: 753×753 (both series), spacing 0.371mm (vs ITK 0.5462mm)
- Mapping used upsampled spacing/dims with ITK direction → ~0.6mm drift
- Z mapping: click at Z=-744.89 (slice 0 of 201) → mapped to Z=-741.76 (slice 0 of 202)
...
#### MRI Test: Brain MRI — Series 5 (Axial T2) ↔ Series 7 (Sagittal T1)
...
Series 5 (axial T2, oblique):
  ...
  Direction = [[ 0.999,  0.027, -0.044],
               [-0.030,  0.997, -0.070],
               [ 0.042,  0.072,  0.997]]
  Patient center ≈ (-2.81, -15.48, 13.41)
...
Series 7 (sagittal T1, oblique):
  ...
  Direction = [[ 0.029,  0.070, -0.997],      ← voxel-X ≈ -patient-Z (sagittal!)
               [ 1.000, -0.002,  0.029],      ← voxel-Y ≈ patient-X
               [ 0.000, -0.998, -0.070]]      ← voxel-Z ≈ -patient-Y
  Patient center ≈ (-132.44, -15.03, -10.48)
...
| Y (A/P) | [-123, +92] | [-116, +86] | [-116, +86] = 202mm ✅ |
| Z (S/I) | [-61, +88] | [-125, +104] | [-61, +104] = 165mm ✅ |

**Observed:**
- Click at vtk_world=(-31.74, 44.09, -28.18) → patient=(-31.57, -64.86, -21.71)
...
- Mapped Z = 103.73 (clamped to origin) → always slice 0

**Root cause: Not a code bug — the sagittal FOV is offset far to the left hemisphere.**
...
### 2026-02-07 — Display upsampling spacing bug (v1.09 → v1.09.1)
...
extent_y_vtk = (831-1) * 0.264  = 219.12mm   ← 0.53mm MISMATCH
```

When this 0.53mm error is multiplied through the direction matrix (which rotates
axes), it compounds — particularly for oblique or sagittal orientations where
...
### 2026-02-07 — Centre-of-slice offset fix (v1.09.2)

**Problem:** The user reported CT sync is "slightly lower than it should be."

**Analysis of CT logs (identity direction, Series 201 3.75mm ↔ Series 202 0.625mm):**

The X,Y coordinates pass through the round-trip **unchanged** (113.05 → 113.05,
9.53 → 9.53). This is mathematically correct for identity-direction CT where both
...
```
Thick slice 14, face position:  Z = -744.89 + 14 × 3.75 = -692.39 mm
Centre of thick slice 14:       Z = -692.39 + 3.75/2     = -690.515 mm

Mapped to thin series (0.625mm):
  From face   (-692.39): ijk_z = (-692.39 + 741.76) / 0.625 = 78.99 → slice 79
  From centre (-690.52): ijk_z = (-690.52 + 741.76) / 0.625 = 81.99 → slice 82
```
...
```python
slice_axis = orientA        # 0→X, 1→Y, 2→Z
...
visually perceives the anatomy. For thin→thick mapping the offset is negligible
...
### 2026-02-07 — Display extent Y-flip mismatch + MRI analysis (v1.09.3)

**Problem:** MRI sync (axial T2 → sagittal T1) was reported "completely wrong."
...
dimensions, so `(display_dims-1) * display_sp ≠ (itk_dims-1) * itk_sp`.

```
Series A (axial T2):
  extent_y_itk   = (448-1) × 0.5134 = 229.49 mm
  extent_y_disp  = (975-1) × 0.229  = 223.05 mm
  MISMATCH: 6.44 mm
```
...
The sagittal T1 covers patient X from −77 to −209 (left hemisphere) while the
axial T2 covers X from −82 to +76.  Overlap is only 5mm wide at the extreme
right of the axial image.

All user clicks had patient X ≈ −31 (right hemisphere), which is 46mm outside
the sagittal volume.  The sync correctly identified this as OUT_OF_BOUNDS
(ijk_z = −5.1), clamped to slice 0, and projected the in-plane coordinates
...
**Also fixed:** `SyncPointInteractorStyle` missing `update_slice` — added
...
| v1.09.4 | Added D_vtk parameter to undo VTK direction before D_itk | **WRONG** — introduced spurious rotation for MRI |
...
Wait — display has upsample: display_sp_z = 5.5 (unchanged), origin same.
Actually: slice 14 in display corresponds to idx_z = 14 → Z = -61.18 + 14*5.5 = 15.82

Rechecking with display index → the Z of -16.61 comes from a different
slice or a different click. But the pattern consistently matches:
  Z = origin_z + slice_index * spacing_z
```

**Definitive test — patient coordinate comparison:**
...
patient = origin + D_itk @ s
        ≈ (-81.84, -123.02, -61.18) + D * (106.19, 171.80, 44.57)
        ≈ (24.35, -66.95, -16.61)   ← matches log output ✓
```

With D_vtk (v1.09.4 approach):
```
D_vtk_inv @ delta → rotated delta → completely different patient coords
patient ≈ (-7, -211, -150)          ← physically impossible, outside body ✗
```
...
```
VTK world (source)
  → display index         (world - origin) / spacing
  → flipped-LPS           IPP + idx[0]*sp*col + idx[1]*sp*row
  → undo flip-Y           rl_apply_flip_y_in_plane (self-inverse)
  → true patient LPS
  → closest target slice  dot(P - IPP_0, normal) / ds
  → project onto plane    P - dot(P - IPP_k, n) * n
  → flip-Y for display    rl_apply_flip_y_in_plane
  → target index          rl_lps_to_target_index
  → VTK world (target)    origin + index * spacing
```
...
instance_number order ≠ IPP order.  The function has been removed entirely
...
**CT (shoulder, Series 201 3.75mm ↔ Series 202 0.625mm):**
- Log shows `[SYNC MAP DICOM]` → primary path active
- X,Y pass-through unchanged (identity direction)
- Z maps correctly: slice 14 centre → correct thin slice ✓
- No drift, no offset

**MRI (brain, Series 5 axial T2 ↔ Series 7 sagittal T1):**
- Log shows `[SYNC MAP DICOM]` → primary path active
- Clicks in overlapping region (patient X < -56) → correct mapping ✓
- Clicks outside overlap → `OUT_OF_BOUNDS` with correct clamping ✓
- Sync dot coincides with reference line ✓
...
| `viewer_2d.py` | ~~`_sort_metadata_instances()` removed in v1.9.8.1~~ — metadata stays in DB order | 0 (reverted) |
...
2. **Pick a concrete test point** and trace it numerically through every stage.
   Don't trust symbolic reasoning — do the arithmetic.
