...
| FIX-007 | Stability/Qt-lifetime | high | **fixed** | **other-PC crash**: `animate_hover`/`animate_active` made a local `QPropertyAnimation` with no parent/ref → GC'd while running → access violation on fast tab hover; now one parented+referenced reusable animation (patient_tab + service_tab) |
| OBS-009 | Networking/Environmental | low | observed | other-PC `download_diagnostics` ~337 errors = server/network (`WinError 10060` timeouts, "no response received", "connection lost") **not a client bug**; app.log/db clean → check that PC's link to the PACS server |
| FIX-008 | Stability/Qt-lifetime | high | **fixed** | **pc2 crash**: `_hp_layout._hide_loading_overlay` built an opacity `QPropertyAnimation` on a possibly-deleted overlay (async-open teardown race) → access violation; now liveness-guarded (`shiboken6.isValid`) + try/except→hide, like `loading_overlay.py` |
| OBS-010 | Stability/CrashEscalation + Multi-study (pc2) | medium | observed | **multi-study FUNCTIONAL = healthy** (grouped render 2–3 studies, sibling prefetch, `grew=` gate all correct — no missing/cross-patient/refetch-loop); crashes = `notify()` re-raising event-dispatch exceptions into a PySide cascade, triggered by **network failures during open** (`WinError 10065/10060`). Several sites already fixed in source → **rebuild pc2** |
| FIX-009 | Performance/OpenLatency (disk load) | high | **fixed** | **46370 (62 series, all local) opened in ~44s** — main thread re-read every DICOM header **sequentially** off a slow/I-O-bound disk (~69 ms/file; server-opened study skips DB path = H1). New **adaptive** `_read_header_stubs`: sequential on fast SSD (no regression), thread-overlapped on slow disk (4.6× in a 30 ms/file sim, byte-identical). Server new-series check already concurrent + default-on. `AIPACS_HEADER_SCAN_PARALLEL=auto` |
| FIX-010 | Sync/DownloadManager (stale state) | high | **fixed** | **46640**: study downloaded at 1 series → DM state `COMPLETED`; server grew to 11 → R17 rejected new series (`Download already exists (Status: Completed)`) so they never downloaded. Fix: when the fresh server list shows missing series, reset the stale **terminal** DM state (`state_store.reset`) so R17 allows the new series; resume skips already-local files. `AIPACS_OPEN_RESET_STALE_COMPLETE` |
| REVIEW-SYNC-001 | Architecture/Client-Server-Sync | medium | observed | Full **download→sync→open→display lifecycle review** (`docs/reports/SYNC_DOWNLOAD_LIFECYCLE_REVIEW_2026-06-15.md`): pipeline already conforms to most standard patterns; gaps = no unified manifest, load still blocks UI, re-download-on-complete, manual stale check, drag-drop remote download. **Implemented:** read-only `sync_manifest.py` (unified local manifest + local-vs-server decision, 11 tests). **Staged (flag+golden):** S1 skip-download-when-complete, S2 manifest gate, S3 off-thread/preview load, S4 drag-drop trigger, S5 progressive UI, S6 SOP-level |
