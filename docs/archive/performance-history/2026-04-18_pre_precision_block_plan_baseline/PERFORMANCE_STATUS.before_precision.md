...
|---|---|---|
| **Unreleased (2026-04-17)** | Fast drag could still show a subtle image-type shift at scroll stop even when the exact filtered frame was already cached; progressive scrollbar could visually shrink to admitted/downloaded count and regrow on each batch; large FAST stack drags on 100+ slice series still felt too one-by-one | `Lightweight2DPipeline.get_rendered_frame()` now prefers an exact filtered cache hit before an exact unfiltered one during fast interaction, preserving settled-view appearance when the full-quality frame is already free. `_update_vtk_slice_range()` keeps the slider maximum anchored to `_total_expected_slices` during progressive mode while `update_available_slice_count()` / `_is_slice_available()` enforce reachability, and now also preserves the current slider value if a max-range update would otherwise snap it forward. Separately, `QtSliceViewer._get_stack_drag_profile()` now uses a more aggressive large-stack profile (lower threshold, higher bounded cap) so a full-height drag traverses a more meaningful span without changing wheel precision semantics. |
