...
## VTK Viewer Wiring â€” Critical Architecture Rule (v2.3.3 / 2026-04-14)
...
WRONG (causes frozen image):
  ...
  viewer.SetInputData(image_reslice.GetOutput())   â†گ trivial producer around reslice output

CORRECT (pydicom_2d path):
  viewer.SetInputData(lazy_vtk_image_data)         â†گ direct to raw numpy-backed source
```
...
Calling `image_reslice.Modified()` in the scroll path does NOT fix this â€” it
only sets the filter's own MTime, which the trivial producer never checks.

### Why direct wiring works

```
1. Lazy decoder fills numpy_array[N]  â†’  data present in lazy_vtk_image_data
2. mark_vtk_modified()  â†’  lazy_vtk_image_data.Modified()  â†’  MTime increases
3. Render()  â†’  trivial producer (wrapping lazy source) detects MTime change
              â†’  re-reads numpy scalars  â†’  correct image at SetSlice(N)
```
...
|------|--------------|-----------------|--------|
| `_preprocess_vtk_image_data` | âœ… Run | â‌Œ **Skip** | Creates a NEW disconnected copy (via `vtkImageResample`); `mark_vtk_modified()` on original has no effect on the copy |
| `image_reslice.GetOutput()` as viewer input | âœ… Use | â‌Œ **Bypass** | Trivial producer never propagates upstream; decoder updates never reach viewer |
| `image_reslice.Modified()` in scroll path | âœ… Harmless | â‌Œ **Do not add** | Silently re-introduces freeze regression |
| `image_reslice.Update()` per slice | N/A | â‌Œ **Do not add** | Not needed; bypasses the whole point of lazy decoding |
...
the rebuild branch and the reconnect block â€” otherwise a series switch re-connects
...
|-------|---------|---------|------------|-----|
| 1 | v2.2.5 | Scroll freezes permanently after progressive grow | `vtkImageReslice.SetInterpolationMode*()` called during scroll â€” collapsed slice range to 1. See `viewer-pipeline.md` آ§Scroll Guardrails. | Never call reslice methods during scroll. Fixed in v2.2.6. |
| 2 (Steps 0â€“3) | early v2.2.8.x | Qt crash during active download â€” `_flush_progressive_grow` unguarded | `QTimer.timeout` fired `_flush_progressive_grow` â†’ `_grow_progressive_fast` â†’ unguarded `_update_vtk_slice_range` / `exit_progressive_mode` calls threw â†’ exception escaped Qt dispatch. | Outer guard on entire `_flush_progressive_grow` body; guards on `update_available_slice_count` and `exit_progressive_mode` calls. 22 tests. |
| 3 (Steps Aâ€“D) | v2.2.9.2 | Qt crash from signal slots | `on_series_images_progress`, `on_series_download_fully_complete`, and `_completion_sweep_tick` had no outer try/except around their Qt signal/timer dispatch bodies. | Outer guard + `_impl` split for all three. `_progressive_grow_timer.stop()` on cleanup. 28 tests. |
| 4 (Steps Eâ€“F) | v2.2.9.3 | Late-phase crash ~3s after download completion, under 83â€“94% CPU | `_on_lazy_slice_ready` outer scope (metrics updates, `int()` type conversions, drop path, final `_log_lazy_metrics_if_due()`) unguarded. Only the render step (lines ~400â€“455) had a try/except â€” the drop path above it did not. `_completion_verify_series` outer function also unguarded (called via `QTimer.singleShot`). | Outer guard + `_impl` split on both. 33 tests. |
...
|---------------|------|----------|--------|
| `QTimer.timeout` signal | `_vc_progressive.py` | `_flush_progressive_grow` | âœ… Guarded (Stage 2, Step 0) |
| Qt signal slot (`seriesProgressUpdated`) | `_vc_progressive.py` | `on_series_images_progress` | âœ… Guarded (Stage 3, Step A) |
| Qt signal slot (`seriesDownloadCompleted`) | `_vc_progressive.py` | `on_series_download_fully_complete` | âœ… Guarded (Stage 3, Step B) |
| `QTimer.singleShot` callback | `_vc_progressive.py` | `_completion_sweep_tick` | âœ… Guarded (Stage 3, Step C) |
| `QTimer.singleShot` callback | `_vc_progressive.py` | `_completion_verify_series` | âœ… Guarded (Stage 4, Step F) |
| Qt signal slot (`slice_ready` from loader) | `_vw_backend.py` | `_on_lazy_slice_ready` | âœ… Guarded (Stage 4, Step E) |
| Qt signal slot (`decode_failed` from loader) | `_vw_backend.py` | `_on_lazy_decode_failed` | âœ… Guarded (Stage 5, Step G) |
| `QTimer.singleShot` (150ms stale-guard restart) | `_vc_progressive.py` | `_grow_progressive_fast` (timer reschedule) | âœ… Protected by outer `_flush_progressive_grow` guard |
| `QTimer.singleShot` (0ms completion) | `_vc_progressive.py` | `on_series_download_fully_complete` (layer callbacks) | âœ… Protected by outer `on_series_download_fully_complete` guard |
...
**Stage 2 (Steps 0â€“3):**
- `_flush_progressive_grow` â†’ outer try/except wrapping entire body (`_vc_progressive.py`)
- `_update_vtk_slice_range` â†’ guard on `update_available_slice_count` call (`_vc_cache.py`)
- `exit_progressive_mode()` bare calls in stale-exhausted and completion branches (`_vc_progressive.py`)

**Stage 3 (Steps Aâ€“D):**
- `on_series_images_progress` â†’ split into outer guard + `_on_series_images_progress_impl` (`_vc_progressive.py`)
- `on_series_download_fully_complete` â†’ split into outer guard + `_on_series_download_fully_complete_impl` (`_vc_progressive.py`)
- `_completion_sweep_tick` â†’ split into outer guard + `_completion_sweep_tick_impl` (`_vc_progressive.py`)
- `_progressive_grow_timer.stop()` added to `clear_all_caches_for_close` and `exit_patient_widget`

**Stage 4 (Steps Eâ€“F):**
- `_on_lazy_slice_ready` â†’ split into outer guard + `_on_lazy_slice_ready_impl` (`_vw_backend.py`)
...
- `_completion_verify_series` â†’ split into outer guard + `_completion_verify_series_impl` (`_vc_progressive.py`)

**Stage 5 (Step G):**
- `_on_lazy_decode_failed` â†’ split into outer guard + `_on_lazy_decode_failed_impl` (`_vw_backend.py`)
  - Outer guard logs 4 context fields: viewer id, reason, backend, loader presence
  - Missed in Stage 4 sweep â€” added in v2.2.9.4
...
home_download_service.on_series_completed
    â”‚   emits series_images_progress(sn, total, total)  [Layer 2a pulse]
    â–¼
ViewerController.on_series_images_progress  [GUARDED â€” Step A]
    â”‚   downloaded >= total â†’ _on_series_download_fully_complete
    â–¼
ViewerController.on_series_download_fully_complete  [GUARDED â€” Step B]
    â”‚   final loader.grow() + slider update on ALL viewers showing series
    â”‚   exit_progressive_mode() on each viewer
    â”‚   _progressive_series.pop(sn)
    â”‚   schedules QTimer.singleShot(500, _completion_verify_series)  [Layer 3]
    â”‚   registers sn into _completion_sweep_series_set  [Layer 4]
    â–¼
ZetaBoost.invalidate_series(sn)  â†گ fires ~0ms post-completion
    â”‚   promotes lazy volume to ZetaBoost L1 cache
    â”‚   starts booster prefetch burst for slices 0..N-1
    â”‚   CPU load: 83â€“94% typical during burst (heavy decode activity)
    â–¼
PyDicomLazyVolume.slice_ready signal (per decoded slice, many rapid-fire)
    â”‚   connected to VTKWidget._on_lazy_slice_ready  [GUARDED â€” Step E]
    â”‚   under high CPU, any exception in type conversions / metrics / drop path
    â”‚   is now captured instead of aborting the process
    â–¼
QTimer fires at +500ms: _completion_verify_series  [GUARDED â€” Step F]
    â”‚   disk count vs viewer count check; catch-up grow if needed
    â”‚   up to 3 retries at 500ms intervals
    â–¼
_completion_sweep_timer fires at 3s intervals  [GUARDED â€” Step C]
    â”‚   final safety net; polls until viewer count matches disk
    â–¼
STABLE â€” all slices visible, progressive mode exited, ZetaBoost caching active
```

**Critical timing note:** ZetaBoost fires immediately at completion (`invalidate_series` â†’ booster starts). This saturates CPU with decode work at the exact moment the `_on_lazy_slice_ready` signal fires repeatedly. This is why late-phase crashes occurred under high CPU but not during early progressive grow (fewer concurrent callbacks).
...
|----------|-----------|------------------------|
| Scroll during download | `_flush_progressive_grow` â†’ `_grow_progressive_fast` â†’ `loader.grow()` | Exceptions in grow must not reset slider max or current_slice |
| Scroll after download | `_on_lazy_slice_ready` render path â†’ `_call_image_viewer_set_slice` | Outer guard must not swallow normal render; `_on_lazy_slice_ready_impl` must execute on happy path |
| Progressive grow | `_progressive_grow_timer` (150ms) â†’ `_flush_progressive_grow_impl` â†’ `_grow_progressive_fast` | Info dict `pending_downloaded` preserved after exception so timer retries |
| pydicom_2d direct VTK wiring | `SetInputData(raw_lazy_vtk)` in `ImageViewer2D.__init__` and `reset_image_viewer` | Never wire through `image_reslice.GetOutput()` for pydicom_2d â€” causes frozen image. See Rule 11 in `IMAGE_PIPELINE_REFERENCE.md`. |
| ZetaBoost promote on completion | `on_series_download_fully_complete` â†’ `zeta_boost.invalidate_series` | Must fire AFTER exit_progressive_mode; outer guard must not suppress this call |
...
### Investigation Notes â€” Stage 4 Hypothesis

#### Why `_on_lazy_slice_ready` is the prime suspect

Three converging indicators (not a confirmed traceback â€” this is the hypothesis that drove Stage 4):
...
2. **Code inspection**: The outer scope of `_on_lazy_slice_ready` (before the render try/except and in the drop path) contains multiple unguarded type conversions:
   - `self._lazy_metrics["decode_ms_total"] += decode_ms_f` â€” `KeyError` if `_lazy_metrics` cleared by `_release_bound_lazy_loader()` racing signal delivery
   - `int(self._lazy_requested_generation)` in `should_render_ready_slice()` args â€” `TypeError` if attribute is `None` during series switch race
   - `int(self._series_generation_id)` same â€” same risk
   - `self._lazy_drop_log_counter = int(self._lazy_drop_log_counter or 0) + 1` in drop path â€” `TypeError` if `_lazy_drop_log_counter` is in unexpected state
   - `self._log_lazy_metrics_if_due()` after the render try/except â€” `RuntimeError` if internal state torn down

3. **CPU/timing context**: ZetaBoost fires a prefetch burst for slices 0â€“98 immediately at completion. This saturates CPU at 83â€“94% and generates 99 rapid-fire `slice_ready` signals. Under this load, any attribute access on partially-modified state (series switch cleanup racing decode delivery) is more likely to throw than during normal single-signal delivery.
...
- ZetaBoost completed safely at `INVALIDATE â†’ PROMOTE` before the crash (confirmed in Log 3 at 16:30:40.182)
- All DMâ†’viewer signal slots (`on_series_images_progress`, `on_series_download_fully_complete`) were already guarded in Stage 3 (Steps Aâ€“B) and confirmed working in Log 3 (`_on_series_download_fully_complete_impl` logged, `EXIT series=203 available=99` logged)
- The crash occurred 3 seconds after the DM completed â€” during post-completion viewer interaction, not during the download itself
